...
/**
 * An <code>ImageDecoder</code> for the PDS family of file formats. <br>
 * "SAMPLE_INTERLEAVED", "BAND_SEQUENTIAL" and "LINE_INTERLEAVED" grey scale
 * images, and color images. The decoder understands Images are created in their
 * native data format. <BR>
 ...
 * This decoder employs the <code>VicarIO libraries</code>. There is no
 * ImageDecodeParam class defined for this decoder. The decoder defaults to the
 * simplest common cases. <br>
 * A single banded image is decoded as grayscale. <br>
 * A 3 banded image is decoded as an RGB color image <br>
 * The decoder will decode images with any number of bands. It is up to the
 * display program to extract the bands to display. The JAI "BandSelect"
...
public class PDSImageDecoder extends ImageDecoderImpl {
	 ...
	 */
	public PDSImageDecoder(SeekableStream input, ImageDecodeParam param) {
		super(input, param);
