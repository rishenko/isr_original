 ...
 * 
 * <p>
 * The PDS file format includes "SAMPLE_INTERLEAVED", "BAND_SEQUENTIAL" and
 * "LINE_INTERLEAVED" grey scale images, and BIP for color images. When writing
 * the source data out, the encoder chooses the appropriate file variant based
 * on the actual SampleModel of the source image. In case the source image data
 * is unsuitable for the PDS file format, for example when source has other than
 * 1 or 3 bands, the encoder throws an Error. <BR>
 * A PDS compliant image label is written to the file before the image data. The
 * data type of the input RenderedImage is written to file.
 * 
 */
public class PDSImageEncoder extends ImageEncoderImpl {

	private static final Logger logger = LoggerFactory.getLogger(PDSImageEncoder.class);

	Document pdsDOM = null; // needed to write a PDS compliant image label

	public PDSImageEncoder(OutputStream output, ImageEncodeParam param) {
		super(output, param);
		// logger.debug("VicarImageEncoder constructor ");

	}

	/**
	 * Encodes a RenderedImage and writes the output to the OutputStream
	 * associated with this ImageEncoder. The RenderedImage can come from ANY
	 * source. The PDS label is constructed based on the image. No metadata is
	 * included in the image encoding.
	 * 
	 * @param im
	 *            the RenderedImage to encode
	 */
	public void encode(RenderedImage im) throws IOException {
		...
		logger.debug("********************************");
		logger.debug("PDSImageEncoder encode ++++++++++++++++++++++++");
		 ...
		 * DataBuffer.TYPE_DOUBLE)) { logger.debug(
		 * "Source image has float/double data type, we don't do that yet ;-)");
		...
		if (numBands == 1 || numBands == 3) {
			...
			logger.debug("Source image has unsuitable number of bands (" + numBands + ") for PDS file format.");
			throw new RuntimeException("Source image has unsuitable number of bands for Vicar file format.");
		}

		logger.debug("VicarEncoder width=" + width + "  height=" + height);
		logger.debug("sampleSize[0]=" + sampleSize[0] + "  numBands=" + numBands + "  dataType=" + dataType);
		logger.debug("minX=" + minX + "   minY=" + minY);

		if (variant == false) {
			// logger.debug("ERROR currently we ONLY handle 8bit grayscale/color images");
			logger.debug("currently we ONLY handle 1 or 3 band grayscale/color images");
			// throw new RuntimeException(
			// "ERROR currently we ONLY handle 8bit grayscale/color images");
		...
		case DataBuffer.TYPE_BYTE:
			...
			format = "BYTE";
			break;
		case DataBuffer.TYPE_SHORT: // do we need to treat ushort and short
									// differently???
		...
		logger.debug("PDSImageEncoder.encode() using vicarIO");
		...
		// now write the data to the image, write the whole image as a single
		// tile
		int startX = 0;
		int startY = 0;
		int x_off = 0;
		...
		Raster tile;
		DataBuffer db;
		// logger.debug("VicarEncoder writeTile() width="+width+"  height="+height
		// );
		...
		for (tj = tymin; tj <= tymax; tj++) {
			for (ti = txmin; ti <= txmax; ti++) {
				...
				// logger.debug("writeTile ti="+ti+" tj="+tj+"  tx="+tx+" ty="+ty
				// );
	 ...
	 * to controll writing the image data to the file. <br>
	 ...
	 * 
	 * @param im
	 *            the RenderedImage to create the label for
	 * 
	...
	public jpl.mipl.io.vicar.SystemLabel createSystemLabel(RenderedImage im) {
		...
		logger.debug("height=" + height + "  width=" + width + "  bands=" + bands);
		logger.debug("dataElements=" + elements + "  b0size=" + b0size);
		for (int i = 0; i < sampleSize.length; i++) {
			logger.debug(" sampleSize[" + i + "]=" + sampleSize[i]);
		}
		...
		logger.debug("PDSEncoder SystemLabel:");
