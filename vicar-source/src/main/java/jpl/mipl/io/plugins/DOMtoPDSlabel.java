 ...
 * 
 * This will be called from inside a writer. The Document used to drive the
 * output may have come from a reader, transcoder, or be created
 * programatically. <BR>
...
public class DOMtoPDSlabel {
	...
	/************************************************
	 * lineMax is the maximum number of characters allowed on a line lineMax
	 * INCLUDES the CR LF end of line. Therefore 78 prinatble characters are
	 * allowed on a line. if the line would be over lineMax the text is chopped
	 * at the nearest word boundary. This wrapped line will begin at wrapStart
	 * indentMax is the number of characters keywords will have including spaces
	 * before the = sign valueMax is the maximum number of charatcters in the
	 * value portion of a line. longValueMax is the length of an unusually long
	 * word portion which will be broken into parts, a hyphen will end an part
	 * to indicate continuation
	 * 
	 * example:
	 * 
	 * PRODUCER_INSTITUTION_NAME = "MULTIMISSION IMAGE PROCESSING LABORATORY,
	 * JET PROPULSION LAB" ^ indentMax <-------+indentMax+---------+-> ^ lineMax
	 * ^ wrapStart
	 * 
	 * ^ ^ <-------+ valueMax+---------+---------+-->
	 *******************************************/
	...
	int imageSize = 0;
	int bands = 1;
	int fileRecords = 0; // this is total number of records in the file,
							// includes all labels and image
	int imageRecords = 0; // this is the number of records in the image

	String pds_ptr = null; // used to set and/or override the values of POINTER
							// objects in the label
	...
	public void display() {
		...
		if (debug)
			System.out.println("PDSInputFile " + xPath + "  >" + nodeValue + "<");
		if (nodeValue == "") {
		...
		if (debug)
			System.out.println("PDSInputFile.display() BANDS=" + bands);
		...
		if (debug)
			System.out.println("PDSInputFile.display() BAND_STORAGE_TYPE=" + band_storage_type);
	...
	public String toString() {
		...
		/*
		 * go thru the process twice, the first time calculatingSize is true.
		 * The values for the label size items is calculated. The second time
		 * thru these values are used in the label creation
		 */
		if (debug) {
		...
		/***
		 * // add at least 300 bytes to the label size so there is space for a
		 * few values to be added next time thru
		 */
		...
		/**
		 * if (recordsToAdd) labelRecordCt += recordsToAdd * 2; } else {
		 * labelRecordCt += recordsToAdd ; }
		 **/
		/*
		 * if (z < 100) { labelRecordCt += 2; } else { labelRecordCt++; }*
		 */
		...
		labelSize = labelRecordCt * record_bytes;
		vicarLabelStartByte = labelSize; // start the vicar label after the PDS
											// label
		// vicarLabelStartByte++;
		...
		if (debug) {
			System.out.println("labelRecordCt=" + labelRecordCt + " labelSize=" + labelSize + "  imageLines="
					+ imageLines);
	...
	String getPadding(String s, int indent) {
		...
		/**
		 * String r = ""; for (int i=1 ; i < x ; i++ ) { r = r+" "; } return r;
		 **/
	}
	 ...
	 * 
	 * @param x
	 *            int, number of spaces to put in the pad String
	...
	/**
	 * Breaks a String into a vector where each element is a word. Uses spaces
	 * and commas to break up the String. If a word is longer than valueMax
	 * characters then the word is broken into pieces that are valueMax long. A
	 * hyphen is added to the end of the broken up words. The hypen is printed
	 * in the label and should be ignored by PDS label readers as a line
	 * continuation character.
	 * 
	 * @param value
	 *            The String to be chopped up
	...
	Vector chopValueString(String value) {
		...
		if (debug)
			System.out.println("### chopValueString " + value);
		StringTokenizer st;
					...
					do {
						...
						if (ti == endi) {
							...
							if (debug) {
								...
								System.out.println("  ##  ti=" + ti + "  endi=" + endi + "  tokLen-endi="
										+ (tokLen - endi));
							}
						} else {
							v.addElement(sb.toString() + "-");
	...
	/*
	 * Used to get the value from an "item" or "subitem" node Will look for
	 * attributes to create a String with the value assume this is an item with
	 * NO subitems OR it is a subitem
	 */
	private String getNodeValueString(Node node) {
		...
		if (attrUnitValue != null) {
			...
			value = value + " <" + attrUnitValue + ">";// append the units value
														// (capitalize ???)
		}
	...
	private String imageHeaderObject() {
		...
		if (vicarLabelString != null && vicarLabelBytes > 0) {
			if (debug)
				System.out.println("imageHeaderObject vicarLabelBytes=" + vicarLabelBytes);
			...
			// if (sb != null) {
			// sb.append(nodeName+pad+"= "+labelRecordCt+LineEnd); }
	...
	/*
	 * creates a String with an OBJECT = IMAGE <br> If the input image data has
	 * not been modified in any way there will be an IMAGE OBJECT in the
	 * Document passed to this class. It will usually include some calculated
	 * image statistics. <br> In the case where no IMAGE OBJECT is in the input
	 * Document one will be created here to be added to the PDS label.
	 */
	private String imageObject() {
		String s = "";
		if (imageObjectFound) {
			// this is only valid if the images data has not been modified in
			// any way
	...
	/**
	 * Creates a hash table of key, value pairs froma pds_ptr string The String
	 * is expected to be 1 or more "key=value" pairs If there are more than one
	...
	private Hashtable getPds_ptrHash(String s) {
		...
		if (s != null) {
			// split on commas,
			StringTokenizer st1 = new StringTokenizer(s, ",", false); // return
																		// delimiter
	...
	private void c80(String msg, String nodeName, String nodeValue, int wrapStart, String pad) {
		...
		// if (x > 80 || wx > 80) {
		if (nodeName.startsWith("TELEMETRY_") || nodeName.equalsIgnoreCase("SPICE_FILE_NAME")
				|| nodeName.equalsIgnoreCase("PRODUCT_ID") || nodeName.startsWith("INST_CMPRS_SEG")
				|| nodeName.equalsIgnoreCase("DATA_SET_ID")) {

			System.out.println("c80 " + msg + " " + nodeName + " = " + nodeValue);
			System.out.println("  x=" + x + " wx=" + wx + "  wrapStart=" + wrapStart + "  plen=" + plen + "  vlen="
					+ vlen);
			// System.out.println(" ");
		}
	...
	private void displayMetadata(Node node, int level) {
		...
		if (nodeName.equalsIgnoreCase("PDS_VERSION_ID")) {
			...
			/**
			 * it will probably be better if the outputFilename is set directly.
			 * That way it will be set at the time of writing the file The
			 * filename should be the one used when the file is written
			 ***/
			pad = getPadding(nodeName, 0);
			if (debug) {
			 ...
			 * into the output label "^IMAGE" WILL BE OUTPUT the ^ is an illegal
			...
			/*
			 * add in a Vicar Label pointer if there is a String set for the
			 * Vicar Label
			 */
			if (sb != null) {
				sb.append(LineEnd);
			...
			if (pds_ptrs_hash != null) {
				...
				while (keys.hasMoreElements()) {
					...
					if (key.equalsIgnoreCase("IMAGE")) {
						...
						if (debug)
							System.out.println("image_ptr = " + image_ptr);
					} else if (key.equalsIgnoreCase("IMAGE_HEADER")) {
						image_header_ptr = Integer.parseInt(value);
						// now remove it from the has
						// pds_ptrs_hash.remove(key);
						if (debug)
							System.out.println("image_header_ptr = " + image_header_ptr);
						if (vicarLabelString == null && detachedLabelOnly) {
							nodeName = "^" + key;
							pad = getPadding(nodeName, 0); // this should be at
															// level 0
						...
						if (detachedLabelOnly) {
							// this is the record the image starts at (1 is the
							// first record, not 0)
							sb.append(nodeName + pad + "= (\"" + inputFilename + "\"," + value + ")" + LineEnd);
			...
			if (vicarLabelString != null && vicarLabelString.length() != 0) {
				...
				// we could check LBLSIZE too to be sure, it would also confirm
				// this is
				...
				if (detachedLabelOnly) {
					vicarLabelStartRecord = 1; // vicar label is the start of
												// inputFilename
				...
				if (sb != null) {
					...
					if (detachedLabelOnly) {
						...
						sb.append(nodeName + pad + "= (\"" + inputFilename + "\"," + vicarLabelStartRecord + ")"
								+ LineEnd);
					} else if (outputFilename != null) {
						sb.append(nodeName + pad + "= (\"" + outputFilename + "\"," + vicarLabelStartRecord + ")"
								+ LineEnd);
					} else {
			...
			if (sb != null) {
				...
				if (detachedLabelOnly) {
					...
					// this is the record the image starts at (1 is the first
					// record, not 0)
			...
			if (debug)
				System.out.println("\n" + nodeValue + "\n");
			if (sb != null) {
				sb.append(LineEnd + nodeValue + LineEnd + LineEnd);
			}
		} else if (nodeName.equalsIgnoreCase("VICAR_SYSTEM")) {
			// DON'T PUT THIS IN TO THE LABEL IT MAY BE USED FOR THE EMBEDDED
			// VICAR LABEL
			...
			/*
			 * CLASS is a special type of group/object there should be a comment
			 * in the group print this , then the items in the class there
			 * should NOT be a CLASS END_CLASS used
			 */
			NamedNodeMap map = node.getAttributes();
			if (map != null) {
				int length = map.getLength();
			...
			// "CLASS                        = "+objectName);
			// if (sb != null) {
			// sb.append("CLASS                         = "+objectName+LineEnd);
			// }
			/**
			 * indent(level); nodeName = "CLASS"; pad = getPadding(nodeName,
			 * level); if (sb != null) {
			 * sb.append(nodeName+pad+"= "+objectName+LineEnd); }
			 */
			inObject = true; // used by displayMetadata to know we are inside
								// something
			...
			// level--;
			// System.out.println(
			// "END_CLASS                     = "+objectName);
			// if (sb != null) {
			// sb.append("END_CLASS                     = "+objectName+LineEnd);
			// }
			// if (sb != null) { sb.append(" "+LineEnd); } // add a blank line
			// after a class

			/**
			 * indent(level); nodeName = "END_CLASS"; pad = getPadding(nodeName,
			 * level); if (sb != null) {
			 * sb.append(nodeName+pad+"= "+objectName+LineEnd); }
			 */
			...
			pad = getPadding(nodeName, level);
			if (debug)
				System.out.println(nodeName + pad + "= " + objectName);
			if (sb != null) {
				sb.append(nodeName + pad + "= " + objectName + LineEnd);
			}
			...
			// if (sb != null) {
			// sb.append("OBJECT                         = "+objectName+LineEnd);
			// }
			indent(level);
			nodeName = "OBJECT";
			pad = getPadding(nodeName, level);
			...
			// if (sb != null) {
			// sb.append("END_OBJECT                     = "+objectName+LineEnd);
			// }
			...
			/**
			 * an item contains attributes key, quoted the item node has no
			 * value the value is stored in a text node child or subitems item
			 * or subitems may have units if there are units append to value
			 * enclosed in <>
			 **/
			...
			/*
			 * look for subitems (they are children) construct the value string
			 * from the concatenation of the subitems add quotes and units too
			 */
			nodeName = attrKey;

			NodeList childList = node.getChildNodes();
			// even if there are no children an empty (non NULL) list wiull be
			// returned
			...
			for (int i = 0; i < len; i++) {
				...
				if (childNodeName.equalsIgnoreCase("subitem")) {
					...
					if (i == 0 && len == 1) {
						...
						sval = sval.replaceFirst("^\\s+", ""); // remove any
																// leading
																// spaces
						// sval = sval.replaceFirst("\\s+$",""); // remove any
						// trailing spaces
					...
					nodeValue += getNodeValueString(node); // assume the value
															// is a TEXT node of
															// the "item"
				}
				// now determine how to assemble the string value
				...
				if (hasSubitems == true) {
					...
					// units are already taken care of in
					// getNodeValueString(node)
			...
			if (nodeName.equalsIgnoreCase("FILE_RECORDS")) {
				if (debug)
					System.out.println("FILE_RECORDS band_storage_type = " + band_storage_type);
				nodeValue = getNodeValueString(node);
				...
				if (sb != null) {
					sb.append(nodeName + pad + "= " + fileRecords + LineEnd);
				}
				...
				if (nodeValue.length() < 80 - wrapStart) {
					...
					if (debug)
						System.out.println(nodeName + pad + "= " + s);
					// if (sb != null) { sb.append(
					// nodeName+pad+"= \""+s+LineEnd); }
					int vlen = s.length();
					if (vlen <= valueMax) {
						if (sb != null) {
						...
						if (debug)
							System.out.println("q3 vx=" + vx + " lineMax=" + lineMax + " vlen=" + vlen + " vpad="
									+ vpad + "<");
					...
					for (int i = 1; i < vec.size(); i++) {
						...
						if (i == vec.size() - 1) {
							// if (debug)
							// System.out.println(getWrapStart()+s+"\"");
							if (debug)
								System.out.println(getWrapStart() + s);
							// if (sb != null) { sb.append(
							// getWrapStart()+s+"\""+LineEnd); }
							...
							if (sb != null) {
								...
								if (vlen + wrapStart > longValueMax) {
									...
									vpad = getValuePadding(vx);
									if (debug)
										System.out.println("q5 vx=" + vx + " lineMax=" + lineMax + " vlen=" + vlen
												+ " vpad=" + vpad + "<");
									// sb.append(pad+"\""+s+"\""+LineEnd);
									sb.append(vpad + s + LineEnd);
				...
				if (nodeValue.length() < 80 - wrapStart) {
					...
					if (vlen <= valueMax) {
						...
						sb.append(nodeName + pad + "= " + LineEnd);
						// CR LF end a line and are part of the 80 character
						// count
						int vx = lineMax - vlen - 2;
						vpad = getValuePadding(vx);
						if (debug)
							System.out.println("nq3 vx=" + vx + " lineMax=" + lineMax + " vlen=" + vlen + " vpad="
									+ vpad + "<");
						sb.append(vpad + s + LineEnd);
					} else {
						if (debug)
					...
					for (int i = 1; i < vec.size(); i++) {
						s = (String) vec.elementAt(i);
						// check each of these to see if the value is over 80
						// chars ?
						if (sb != null) {
							...
							if (vlen + wrapStart > longValueMax) {
								...
								vpad = getValuePadding(vx);
								if (debug)
									System.out.println("nq5 vx=" + vx + " lineMax=" + lineMax + " vlen=" + vlen
											+ " vpad=" + vpad + "<");
								// sb.append(pad+"\""+s+"\""+LineEnd);
								sb.append(vpad + s + LineEnd);
