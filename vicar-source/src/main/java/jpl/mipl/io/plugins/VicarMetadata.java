...
/**
 * @version 1.0 jdk1.4 compatable version <br>
 ...
 *          Once the VicarLabel is set in this VicarMetadata Object then
 *          requests for trees will be servered by creating the requested tree
 *          with private methods using the VicarLabel as input. <br>
 *          If there is no VicarLabel set no trees will be returned unless they
 *          have been externally created and then set. a native or common tree
 *          can be externally created and then set.
 *          <p>
 *          TO DO: <br>
 *          2 possible ways to handle output by the writer.<br>
 *          1) Use an XSL transform on the tree to format the stream of bytes
 *          written to the image file as its header.<br>
 *          2) Have a method which will convert a tree (naitive and common) to a
 *          VicarLabel Currently the writer passes a VicarLabel object to the
 *          VicarIO routines and it handles the writing/formating of the label.
 *          <p>
 *          In the transcoder world we should have a label writer which combines
 *          the tree and an XSL script to produce the label output stream. The
 *          transcoder may have used an XSL script to modify the DOM tree before
 *          it is handed to the tree -> VicarLabel converter.
 */
public class VicarMetadata extends IIOMetadata implements Cloneable {
	...
	// or is native really the common one ???
	// org.apache.xerces.dom.DocumentImpl
	// public static final String commonMetadataFormatName =
	// "com.sun.imageio_1.0";

	// public static final String commonMetadataFormatName="xml";// or default
	// or DOM ??
	public static final String[] metadataFormatNames = { nativeImageMetadataFormatName, commonMetadataFormatName };

	// we will hold our "native" metadata in a simple DOM Document object
	// since Document descends from Node we can return it as a Node with no
	// problem
	...
	// our real "native" metadata format is a VicarLabel object created by the
	// reader
	 ...
	 * 
	 * IIOMetadataController controller; IIOMetadataController
	 * defaultController; // setDefaultController() ???
	 **/
	...
	/**
	 * returns the label record count for the vicar label in the input image
	 * file. It is calculated from the values front_label_size / record_length
	...
	/**
	 * set the value for the vicar image label record length (recsize) Will be
	 * set by VicarInputFile when this metadata object is created
	 * 
	 * @param len
	 */
	...
	/**
	 * set the value for the vicar image label size in bytes Will be set by
	 * VicarInputFile when this metadata object is created
	 * 
	 * @param size
	 */
	...
	/**
	 * get the value for the vicar image label size in bytes may be useful for
	 * creating PDS detached labels
	 * 
	 * @return
	 */
	 ...
	 * getNativeTree
	 * 
	 * create or return the tree the native tree is a Document
	 */
	 ...
	 * getCommonTree
	 * 
	 * create or return the tree the common tree is a IIOMetadataNode
	 * 
	 * To do this right I think I also need to supply a IIOMetadataFormat for my
	 * IIOMetadata that I return here
	 */
	private Node getCommonTree() {
		...
		if (commonNode == null) {
			if (vicarLabel != null) {
				...
				/**
				 * // VicarLabelToDOM vl2DOM = new VicarLabelToDOM(vicarLabel,
	...
	/**
	 * this is the first try for "native" metadtata, I don't think it really
	 * fits with the intent ??? should I enclose the VicarLabel Object as a node
	 * of a tree instead?? Maybe in nativeDoc ??
	 **/
	...
	/**
	 * Maybe getAsTree("VicarLabel") returns a tree with one node whose value is
	...
	/**
	 * The VicarBinaryLinePrefix is an Object which holds all the data of the
	 * image It is collected by the reader or set somewhere else. The writer has
	 * the option of putting the prefix data into the output file. A null
	 * VicarBinaryLinePrefix indicates a lack of prefix data
	 * 
	 * @return
	 */
	...
	/**
	 * The VicarBinaryLHeader is an Object which holds the Binary Header data of
	 * the image It is collected by the reader or set somewhere else. The writer
	 * has the option of putting the header data into the output file. A null
	 * VicarBinaryHeaderx indicates a lack of binary header data. The binary
	 * header is a special case. MOST vicar images do NOT have any binary
	 * header.
	 * 
	...
	/************************
	 * 
	 * The Tree is created elsewhere. It is passed in here for VicarMetadata to
	 * hold.
	 * 
	 *************************/
	public void setFromTree(String formatName, Node root) {
		if (debug)
			System.out.println("VicarMetadata.setFromTree() " + formatName);
	...
	/*
	 * there is a problem here public IIOMetadataController getController() {
	 * return controller; }
	 * 
	 * public IIOMetadataController getDefaultController() { return
	 * defaultController; }
	 * 
	 * public booelean hasController() { if (controller != null) { return true;
	 * } else { return false; } }
	 */
