...
/**
 * This class builds a DOM Document from a vicar image label object.<BR>
...
public class VicarLabelToDOM {
	...
	/***
	 * getMetadataFormatNames() Returns an array of Strings containing the names
	 * of the metadata formats recognized by this plug-in's getAsTree,
	 * setFromTree, and mergeTree methods. String getNativeMetadataFormatName()
	 ***/
	...
	public void buildDom() {
		...
		try {
			...
			if (_label == null) {
				...
				// throw new
				// Exception("VicarLabelToDOM: No VicarLabel object available!");
			}
	...
	public String toDom() // make this void ???
	{
		...
		try {
			...
			/*
			 * we don't need this if DOMuitils makes all the Documents they will
			 * all be compatable Class c = _document.getClass(); _documentName =
			 * c.getName();
			 * 
			 * Element documentNameNode = (Element)
			 * _document.createElement(_documentName);
			 * 
			...
			// _document = domUtil.getNewDocument(nativeMetadataFormatName,
			// this.toString());
			...
			// no Document implementations support this feature, it will return
			// null and be ignored
			// eventually it may be taken out altogether, for now it is a
			// placeholder for the thought
			Node userNode = null;
			// userNode = domUtil.createUserNode(_document,
			// "jpl.mipl.io.vicar.VicarLabel", _label);

			/**
			 * Element vicarNode =
			 * _document.createElement("jpl.mipl.io.vicar.VicarLabel"); // this
			 * will only work for the xerces Implementation
			 * org.apache.xerces.dom.ElementImpl e =
			 * (org.apache.xerces.dom.ElementImpl) vicarNode;
			 ...
			 * System.out.println(
			 * "<VicarIIOMetadataNode> "+element.toString()); root.appendChild
			 * (element);
			 **/
	...
	/**
	 * add a node which is a VicarIIOMetadataNode and holds the VicarLabel
	 * object IIOMetadaNode is NOT compatable with the xerces implementation I
	 * am using so I can't add this node to my document or one of its children
	 **/
	...
	public String toDomS(VicarLabelSet set, Node root) {
		...
		for (Iterator it = set.iterator(); it.hasNext();) {
			...
			if (value.indexOf(',') != -1 && value.indexOf('(') != -1) {
				if (debug)
					System.out.println(keyTag + " >" + key + "< value >" + value + "< ");
				...
				for (int i = 0; i < subValues.length; i++) {
					...
					String quoted = "false"; // could also use "false" "single"
												// "double"
					if (subValue.indexOf('"') != -1) {
						quoted = "true"; // "double"
						subValue = subValue.replaceAll("\"", ""); // replace all
																	// double
																	// quotes
				...
				if (value.indexOf('"') != -1) {
					quoted = "true"; // "double"
					value = value.replaceAll("\"", ""); // replace all double
														// quotes
				} else if (value.indexOf('\'') != -1) {
					quoted = "true"; // "single"
					value = value.replaceAll("\'", ""); // replace all single
														// quotes
				} else {
					quoted = "false";
				}

				value = value.trim(); // remove spaces at front and back of the
										// line
