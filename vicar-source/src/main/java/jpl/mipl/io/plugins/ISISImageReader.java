 ...
 * @version 0.5
 */
public class ISISImageReader extends JPLImageReader {
	...
	// private VicarInputFile vif; // this may become ISISInputFile or
	// RawInputFile
	private ISISInputFile iif; // we need a system label object for all the
								// reader routines
	...
	private void readHeader() throws IIOException {
		...
		try {
			...
			setSystemLabel(iif.getSystemLabel());
			...
			if (debug) {
				...
				System.out.println("System label:" + getSystemLabel());
				// System.out.println(getSystemLabel().toString());
				// readHeader(input);
		 ...
		 * System.out.println("IOException Error reading header:"
		 * +ex.getMessage()); ex.printStackTrace(); return; }
		 **/
		catch (Exception ex) {
		 ...
		 * { ISISLabelToDOM pdsLabel2Dom = new ISISLabelToDOM((ImageInputStream)
		 * stream, null); document = pdsLabel2Dom.getDocument();
		 * System.out.println("ISISImageReader new ISISMetadata with document");
		 * System
		 * .out.println("+++++++++++++++++++++++++++++++++++++++++++++++");
		 * pdsMetadata = new ISISMetadata(document); gotMetadata = true; //
		 * return ; } else {
		 * System.out.println("Improper input type, can't read the header "
		 * +input);
		 * System.out.println("ISISImageReader NO ISISMetadata ************");
		 * return; }
		 * 
	...
	throws IIOException {
		...
		if (debug)
			System.out.println("ISISImageReader.read() imageType " + imageType);
		BufferedImage theImage = null;
		...
		if (parameterizedRead == false && imageType != null) {
			...
			// vicarIO
			if (debug)
				System.out.println("iif.createSampleModel(" + width + "," + height + ")");

			sampleModel = iif.createSampleModel(width, height);
			// SampleModel sampleModel = new
			/*
			 * create my own sample model to test if this is the problem with
			 * the current reader (slooooowwwwwww)
			 */
			if (debug) {
			...
			if (bands <= 3) {
				...
				// the BuffereImgae will be from the child so a ColorModel can
				// be created
				...
				/**
				 * theImage = new java.awt.image.BufferedImage(w, h,
				 * BufferedImage.TYPE_CUSTOM); theImage.setData(raster); // this
				 * one will NOT be displayable
				 **/
				// childRaster
