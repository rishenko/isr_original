...
public class PDSImageReaderSpi extends ImageReaderSpi {
	...
	private static final Logger logger = LoggerFactory.getLogger(PDSImageReaderSpi.class);
	...
	private static final String[] MIMEtypes = { "image/x-pds", "image/x-img", "image/pds", "image/img" };
	...
	private static final String readerClassName = PDSImageReader.class.getCanonicalName();

	// private static String[] extraStreamMetadataFormatNames[] = (String[][])
	// "";
	// private static String[] extraStreamMetadataFormatClassNames[] =
	// (String[][]) "";
	// Class names are given as strings to avoid loading classes
	// until they are needed.
	private static final String[] writerSpiNames = { PDSImageWriterSpi.class.getCanonicalName() };

	// private static String[] extraImageMetadataFormatNames[] = (String[][])
	// "";
	// private static String[] extraImageMetadataFormatClassNames[] =
	// (String[][]) "";
	...
	public PDSImageReaderSpi() {
		super(vendorName, version, names, suffixes, MIMEtypes, readerClassName, STANDARD_INPUT_TYPE, writerSpiNames,
				_supportsStandardStreamMetadataFormat, // new in 1.4
				// new add Metadata stuff
				PDSMetadata.nativeStreamMetadataFormatName, PDSMetadata.nativeStreamMetadataFormatClassName,
				...
				PDSMetadata.nativeImageMetadataFormatName, PDSMetadata.nativeImageMetadataFormatClassName,
	...
	public void onRegistration() {
		logger.debug("PDS reader spi: on registration");
	}
	...
	public boolean canDecodeInput(Object input) throws IOException {
		...
		if ((input instanceof ImageInputStream)) {
			...
			// do I need to close this stream ???
			stream = new FileImageInputStream(new File((String) input));
		} else {
			logger.debug("could not decode input: {}", input);
			return false;
		}
		...
		boolean ret = ((header[0] == 'P') && (header[1] == 'D') && (header[2] == 'S') && (header[3] == '_')
	...
	public ImageReader createReaderInstance() {
		logger.debug("createReaderInstance PDSImageReader");
		return new PDSImageReader(this);
	}

	public ImageReader createReaderInstance(Object extension) {
		// we'll ignore this till we know what it does
		logger.debug("createReaderInstance extension={}", extension);
		// throw new IllegalArgumentException
		// ("Vicar does not support extensions");
		// find out what an extention might be
		return new PDSImageReader(this);
	}

	/*
	 * unimplemented interfaces .... public Class[] getInputTypes() { // return
	 * an array of all the inputs we can handle // should include things we can
	 * "wrap" to look correct ImageInputStream // we wrap this to look like
	 * SeekableStream SeekableStream // if we learn how to use the new ones IIO
	 * added list them too // }
	 */
