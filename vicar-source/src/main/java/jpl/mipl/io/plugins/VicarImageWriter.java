 ...
 * @version 0.6
 * 
 *          changed the Object returned by getDefaultWriteParam() from
 *          ImageWriteParam to PDSImageWriteParam this allows the writer to be
 *          passed some values in PDSImageWriteParam
 */
public class VicarImageWriter extends ImageWriter {
	...
	public ImageWriteParam getDefaultWriteParam() {
		...
		// this allows us to use the extra info carried by the
		// PDSImageWriteParam
	 ...
	 * out as Vicar. Based on the RenderedImage create a VicarLabel.
	 **/
	...
	private void write(RenderedImage ri, VicarLabel vicarLabel) {
		...
		try { // catch Exceptions for all the VicarIO methods called
			if (vicarLabel == null) {
				...
				// we can't rely on the System part in the VicarLabel since the
				// format and other System
				// attributes may have changed after the data was read into
				// memory
				if (_debug)
			...
			if (_debug)
				System.out.println("VicarImageWriter.write() vof.open() " + output);
			// vof.open((OutputStream) output); // output stream was supplied to
			// the constructor
			...
			if (_debug)
				System.out.println("VicarWriter write() tileWidth=" + tileWidth + "  tileHeight=" + tileHeight);
			...
			int ti, tj;
			/*
			 * int minTileX = ri.getMinTileX(); int maxTileX = ri.getMinTileX()
			 * + ri.getNumXTiles() - 1; int minTileY = ri.getMinTileY(); int
			 * maxTileY = ri.getMinTileY() + ri.getNumYTiles() - 1;
			 */
