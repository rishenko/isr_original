 ...
 * tries to access the underlying stream directly. Thus, multiple threads can
 * issue simultaneous <code>readRecord()</code> or <code>readTile()</code>
 ...
 * work (the request order would be non-deterministic). For random-hard streams,
 * threads will work but could cause performance hits depending on the ordering
 * of the requests. Random-easy streams should be fine.
 * <p>
 * 
 * @see VicarInputImage
 * @see VicarInput
...
public class VicarInputFile implements VicarInput {
	 ...
	 * <code>DataInput</code>, but specific subclasses may be handled specially.
	 * See the <code>stream_</code>*() functions.
	 */
	...
	private static final Logger logger = LoggerFactory.getLogger(VicarInputFile.class);
	// //////////////////////////////////////////////////////////////////////

	/***********************************************************************
	 * Yucky static initializer that determines if we're on 1.3 or later. 1.3
	 * can't handle double/float in things like BandedSampleModel but 1.4 can.
	 * Also determines JAI 1.1.1 or later... 1.1.2 can handle JDK 1.4's
	 * DataBufferFloat but 1.1.1 can't (note both JDK 1.4 and JAI have a
	...
	/***********************************************************************
	 * Sets the file offset to the given value. The file offset is the amount to
	 * skip at the beginning of the file, before the VICAR label even starts.
	 * The purpose of it is to handle dual PDS/VICAR files; the PDS file
	 * describes the offset where the VICAR file starts. The offset is 0-based.
	 * <p>
	...
	/***********************************************************************
	 * Opens a file given an <code>InputStream</code>. Random access is assumed
	 * to be needed, so the stream is wrapped if necessary.
	 * 
	 * @throws IOException
	 */
	...
	public synchronized void open(InputStream is, boolean sequential_only) throws IOException {
		...
		if (sequential_only) {
			...
			_random_easy = (_input_stream instanceof FileSeekableStream || _input_stream instanceof ByteArraySeekableStream);
		}
	...
	/***********************************************************************
	 * Opens a file given a filename. What about URL's?
	 * 
	 * @throws IOException
	 */
	 ...
	 * <code>open()</code> routine is called based on whether the object is a
	 * <code>String</code> (filename), <code>SeekableStream</code>,
	 * <code>ImageInputStream</code>, <code>RandomAccessFile</code>, or generic
	 * <code>InputStream</code>.
	 * 
	 * @throws UnsupportedOperationException
	 *             if it's an unrecognized type.
	 */
	...
	/***********************************************************************
	 * Does the actual work of opening the file. Reads in the first part of the
	 * label, and sets up the SystemLabel object.
	 * 
	 * @throws IOException
	 */
	protected void openInternal() throws IOException {
		...
		_data_format = new VicarDataFormat(_system.getHost(), _system.getIntFmt(), _system.getRealFmt());
		...
		// if (_data_format.getIntFormatCode() ==
		// VicarDataFormat.REAL_FMT_RIEEE)
		...
		if (_input_stream instanceof ImageInputStream) {
			if (int_order == ImageInputStreamStride.LOW_ORDER && float_order == ImageInputStreamStride.LOW_ORDER)

				((ImageInputStream) _input_stream).setByteOrder(ByteOrder.LITTLE_ENDIAN);
			else
				((ImageInputStream) _input_stream).setByteOrder(ByteOrder.BIG_ENDIAN);
		}

		_input_stream_wrap = new ImageInputStreamStride(_input_stream, int_order, float_order);
	...
	/**
	 * a call to readBinaryHeaderData will read the Binary header and add the
	 * Object. This call allows the VicarBinaryHeader to be set from some where
	 * else
	 * **/
	...
	/**
	 * forces the reading of the Binary header. A VicarBinaryHeader Object is
	 * created and the data is stored in it. The VicarBinaryHeader should be
	 * added to the VicarMetadata Object. Most Vicar files do NOT contain a
	 * binary header.
	 * 
	 */
	protected void readBinaryHeaderData() {
		...
		try {
			...
			if (nlb == 0) {
				...
				// bltype, bhost, bintfmt and brealfmt are all needed by
				...
				logger.trace("Created VicarBinaryHeader nlb=" + nlb + " recsiize=" + recsize);
	 ...
	 * readLinePrefixData
	 * 
	 * Determine if there is any prefix data in this file. If so get the Object
	 * to hold it and read it in. This is called after the header has been read.
	 */
	protected void readLinePrefixData(int nbb) {
		int bandList[] = null; // if a bandList is set we should only read the
								// prefixs of the
		// readPrefixData( null);
	...
	protected void readLinePrefixData(int nbb, int bandList[]) {
		...
		logger.trace("*******************************");
		logger.trace("**");
		logger.trace("** readLinePrefixData");
		logger.trace("**");
		logger.trace("** nbb=" + nbb);
		logger.trace("*******************************");
		...
		// read the data
		// bltype, bhost, bintfmt and brealfmt are all needed by application
		// programs to
		// properly interpret the data in a binary prefix or label
		String bltype = _system.getBLType();
		...
		logger.trace("Created VicarBinaryLinePrefix nl=" + nl + " nb=" + nb + " nbb=" + nbb + " bltype=" + bltype);
		...
		logger.trace("VicarInputFile.readLinePrefixData nb=" + nb + "  nl=" + nl + "  nbb=" + nbb
				+ " +_system.getNBB()=" + _system.getNBB());
		logger.trace(" _system.getNS() " + _system.getNS() + "  _system.getNL() " + _system.getNL()
				+ "  _system.getNB() " + _system.getNB());
		logger.trace(" _system.getN1() " + _system.getN1() + "  _system.getN2() " + _system.getN2()
				+ "  _system.getN3() " + _system.getN3());
		logger.trace(" num_bands=" + num_bands + " bandList ");

		for (i = 0; i < num_bands; i++) {
			logger.trace("    bandList[" + i + "] = " + bandList[i]);
		}
		logger.trace(" org_code=" + org_code + "  SystemLabel.ORG_BSQ=" + SystemLabel.ORG_BSQ);
		logger.trace(" ----------------------------------------------------------------");
		...
		case SystemLabel.ORG_BSQ:
			...
			logger.trace("VicarDataFormat=" + vdf + " HOST=" + _system.getHost());

			VicarBinaryLabel vbl = new VicarBinaryLabel(nbb, vdf);
			logger.trace("ORG_BSQ nbb=" + nbb + "  ****************************");

			for (band_ind = 0; band_ind < num_bands; band_ind++) {
				band = bandList[band_ind];
				logger.trace("band_list loop band_ind=" + band_ind);
				// _vicarBinaryLinePrefix.setPrefix(int band, int line, byte[]
				// b, int size)
				// read the data
				...
				try {
					...
					for (line = 0; line < nl; line++) {
						...
						// logger.trace("readLinePrefix line="+line+" band="+band);
		 ...
		 * 
		 * // case SystemLabel.TYPE_BYTE: for (band_ind=0; band_ind < num_bands;
		 * band_ind++) { band = bandList[band_ind]; bdata =
		 * data.getByteData(band); offset = data.getOffset(band) + line *
		 * data.lineStride; readRecordNS(bdata, x, w, offset, data.pixelStride,
		 * band, line+y); } } break;
		 * 
		 * case SystemLabel.ORG_BIP:
		 * 
		 * 
		 * // Is band_offsets[] needed? Not sure HotSpot is good enough // to
		 * optimize data.getOffset(band_ind) inside the pixel loop... int
		 * band_offsets[] = new int[num_bands]; for (band_ind=0;
		 * band_ind<num_bands; band_ind++) { band_offsets[band_ind] =
		 * data.getOffset(band_ind); }
		 * 
		 * // get starting band and # of bands to read from file // Note that
		 * band_ind indexes into the SM whild band (or // bandList[band_ind])
		 * indexes into the file.
		 * 
		 * int sb = 0; int nb = num_bands; if (bandListGiven) { // get min/max
		 * of bandList int min = bandList[0]; int max = bandList[0]; for (i=1;
		 * i<num_bands; i++) { if (bandList[i] < min) min = bandList[i]; if
		 * (bandList[i] > max) max = bandList[i]; } sb = min; nb = max - min +
		 * 1; }
		 * 
		 * 
		 * // case SystemLabel.TYPE_BYTE: bbdata = data.getByteData(); byte
		 * bbuf[] = new byte[nb]; for (line=0; line < h; line++) { for (samp=0;
		 * samp < w; samp++) { readRecordNS(bbuf, sb, nb, 0, 1, samp+x,line+y);
		 * for (band_ind=0; band_ind < num_bands; band_ind++) { band =
		 * bandList[band_ind]; bbdata[band_ind][band_offsets[band_ind] + samp *
		 * data.pixelStride + line * data.lineStride] = bbuf[band-sb]; } } }
		 * break;
		 **************/
	 ...
	 * extra work once openInternal has completed. Created so PDSInputFile and
	 * ISISInputFile oculd have the oportunity to modify _input_stream_wrap.
	 */
	protected void openInternalLast() {

		// provided for subclasses to put good stuff here
		logger.trace("VicarInputFile.openInternalLast()");
	 ...
	 * overridden by subclasses for different label formats. The method must set
	 * up: _label (can be null), _lblsize_front, _system (cannot be null),
	...
	protected void setupLabels() throws IOException {
		...
		_image_size_bytes = (_system.getNLB() + ((long) _system.getN2() * (long) _system.getN3()))
				* _system.getRecsize();
		...
		/*
		 * Disabled - no need to process them all if nobody asks for them.
		 * Deferred execution, and all that... !!!!If this is ever re-enabled,
		 * take _file_offset into account... if (_random_easy &&
		 * _system.getEOL() != 0) { stream_seek(_lblsize_front +
		 * _image_size_bytes); _lblsize_eol =
		 * _label.readLabelChunk(_input_stream); stream_seek(_lblsize_front);
		 * _label.setReadComplete(true); } Disabled
		 */
	 ...
	 * @see #getSystemLabel()
	 * @throws IOException
	 *             if an EOL read is required, and fails. Will not occur for
	 *             random-easy streams.
	 */
	 ...
	 * read. This can be used with sequential or random-hard streams to
	 * determine whether or not <code>getVicarLabel()</code> will do bad things.
	 * The label will always be completely read for random-easy streams, or for
	 * any stream if there are no EOL labels.
	 * 
	 * @see #getVicarLabel()
	 */
	 ...
	 * <code>VicarLabel</code> object (e.g. a synthetic image made up of
	 * multiple files).
	 */
	...
	/***********************************************************************
	 * Return true if random access to this file is <em>possible</em>. It might
	...
	/***********************************************************************
	 * Return true if random access to this file is <em>easy</em>, meaning that
	 * the underlying stream/file supports seekable random access, as opposed to
	 * drastic means like mark/reset, contents caching, or re-opening.
	 */
	...
	/***********************************************************************
	 * Low-level read function, returns a byte array for the given record. The
	 * most normal call, to read an entire record, would be
	 * <code>x.readRecord(data, 0, 0, 0, 1, line, band);</code>
	 * 
	 * @param data
	 *            The array to read into
	 * @param start
	 *            Starting position in the record. Normally 0 to read the whole
	 *            record.
	 * @param length
	 *            Number of pixels to read. If 0, the rest of the record is
	 *            read.
	 * @param offset
	 *            Offset into the data array at which to put the data.
	 * @param pixelStride
	 *            Offset from one pixel to the next in the array. For example, a
	 *            stride of 1 has all pixels contiguous while a stride of 3
	 *            allows band interleaved pixels in the array (pixels read from
	 *            the file will be put 3 elements apart in the array). This has
	 *            nothing to do with what is read from the file - only where the
	 *            data is put in the array. Note that this value is measured in
	 *            pixels or array elements - not bytes.
	 * @param n2
	 *            "Line number" to read (for BSQ organization). (For BIL: band
	 *            number, for BIP: sample number). The second dimension's index.
	 * @param n3
	 *            "Band number" to read (for BSQ organization). (For BIL or BIP:
	 *            line number). The third dimension's index.
	 * @throws NonSequentialAccessException
	 *             (subclass of IOException)
	 * @throws IOException
	 */
	public synchronized void readRecord(byte[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		readRecordNS(data, start, length, offset, pixelStride, n2, n3);
	}

	/** Non-synchronized version */
	protected void readRecordNS(byte[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		...
		// logger.trace("VicarInputFile.readRecordNS BYTE pos="+pos+" start="+start+" offset="+offset+
		...
		if (_system.getFormatCode() == SystemLabel.TYPE_BYTE) {
			...
			// logger.trace("  _current_file_pos "+_current_file_pos);
		} else
			// !!!! OTHER CASES: MAYBE DONE IN WRAPPER????!!!!
	 ...
	 */
	public synchronized void readRecord(short[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		readRecordNS(data, start, length, offset, pixelStride, n2, n3);
	}

	/** Non-synchronized version */
	protected void readRecordNS(short[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		long pos = calcFilePos(start, n2, n3);
		seekToLocation(pos);
		if (length == 0)
	 ...
	 * must be used, although the data should be interpreted as unsigned. For
	 * this reason, a different method name is needed.
	 * <p>
	 * Follow the See Also link for parameters
	 * 
	 * @see #readRecord(byte[], int, int, int, int, int, int)
	 */
	public synchronized void readRecordUshort(short[] data, int start, int length, int offset, int pixelStride, int n2,
			int n3) throws IOException {
		readRecordUshortNS(data, start, length, offset, pixelStride, n2, n3);
	}

	/** Non-synchronized version */
	protected void readRecordUshortNS(short[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		long pos = calcFilePos(start, n2, n3);
		seekToLocation(pos);
		if (length == 0)
	 ...
	 * @see #readRecord(byte[], int, int, int, int, int, int)
	 */
	public synchronized void readRecord(int[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		readRecordNS(data, start, length, offset, pixelStride, n2, n3);
	}

	/** Non-synchronized version */
	protected void readRecordNS(int[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		long pos = calcFilePos(start, n2, n3);
		seekToLocation(pos);
		if (length == 0)
	 ...
	 * @see #readRecord(byte[], int, int, int, int, int, int)
	 */
	public synchronized void readRecord(float[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		readRecordNS(data, start, length, offset, pixelStride, n2, n3);
	}

	/** Non-synchronized version */
	protected void readRecordNS(float[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		long pos = calcFilePos(start, n2, n3);
		seekToLocation(pos);
		if (length == 0)
	 ...
	 * @see #readRecord(byte[], int, int, int, int, int, int)
	 */
	public synchronized void readRecord(double[] data, int start, int length, int offset, int pixelStride, int n2,
			int n3) throws IOException {
		readRecordNS(data, start, length, offset, pixelStride, n2, n3);
	}

	/** Non-synchronized version */
	protected void readRecordNS(double[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		long pos = calcFilePos(start, n2, n3);
		seekToLocation(pos);
		if (length == 0)
	 ...
	 * expect, and <code>offset</code> must be multiplied by 2.
	 * <code>length</code> is still measured in <em>pixels</em>, so there are 2*
	 * <code>length</code> values filled in the array. pixelStride also applies
	 * to <em>pixels</em>, so there are always two consecutive floats for
	 * real/imaginary, and pixelStride gives the offset between one real and the
	 * next (also between one imaginary and the next).
	 * <p>
	 ...
	 * @see #readRecord(byte[], int, int, int, int, int, int)
	 */
	public synchronized void readRecordComp(float[] data, int start, int length, int offset, int pixelStride, int n2,
			int n3) throws IOException {
		readRecordCompNS(data, start, length, offset, pixelStride, n2, n3);
	}

	/** Non-synchronized version */
	protected void readRecordCompNS(float[] data, int start, int length, int offset, int pixelStride, int n2, int n3)
			throws IOException {
		long pos = calcFilePos(start, n2, n3);
		seekToLocation(pos);
		if (length == 0)
		...
		if (_system.getFormatCode() == SystemLabel.TYPE_COMP) {
			...
			for (int i = 0; i < length; i++) { // simple read
				_input_stream_wrap.readFloats(data, offset + i * pixelStride * 2, 1, 1); // real
				_input_stream_wrap.readFloats(data, offset + i * pixelStride * 2 + 1, 1, 1); // imag
			}
			_current_file_pos += length * _system.getPixelSize();
		} else
			// !!!! OTHER CASES: MAYBE DONE IN WRAPPER????!!!!
	...
	/***********************************************************************
	 * Provides access to the binary header, which is NLB records at the
	 * beginning of the file. Follow the See Also link for complete description
	 * 
	...
	public synchronized VicarBinaryLabel getBinaryHeader() throws IOException {
		...
		if (_binary_data_format == null)
			_binary_data_format = new VicarDataFormat(_system.getBHost(), _system.getBIntFmt(), _system.getBRealFmt());
	 ...
	 * of each record. Follow the See Also link for complete description
	 * 
	 * @see VicarInput#getBinaryPrefix(int,int)
	 */
	public synchronized VicarBinaryLabel getBinaryPrefix(int n2, int n3) throws IOException {
		...
		// logger.trace("getLinePrefix pos="+pos+" size="+size+"  n2="+n2+" n3="+n3);
		// Read the entire header
		stream_read(vbl.getBuffer());
	...
	/********************************************
	 * returns the vicar recsize value fron the system label This would be
	 * useful in creating a PDS detached label
	 * 
	 * @return
	 */
	...
	/**
	 * returns the number of bytes of the vicar front label This would be useful
	 * in creating a PDS detached label to set skip records
	 * 
	 * @return
	 */
	...
	public int getFileRecordCount() {
		...
		return (int) (_lblsize_front + _image_size_bytes + _lblsize_eol) / _record_size;
	}
	 ...
	 * @see VicarInput#getBinaryPrefix(int,int)
	 */
	public synchronized byte[] getBinaryPrefix(int n2, int n3, byte buf[]) throws IOException {
		if (_system.getNBB() == 0) { // no header!
			// / zero out the buffer
	...
	public SampleModel createSampleModel(int tileWidth, int tileHeight) {
		...
		case SystemLabel.ORG_BSQ:
			...
			if (float_type)
				return new ComponentSampleModelJAI(data_buffer_type, tileWidth, tileHeight, pixel_stride,
						scanline_stride, bank_indices, band_offsets);

			/**/
			if (num_bands == 1)
				return new PixelInterleavedSampleModel(data_buffer_type, tileWidth, tileHeight, pixel_stride,
						scanline_stride, band_offsets);
			else
				/***/
				return new BandedSampleModel(data_buffer_type, tileWidth, tileHeight, scanline_stride, bank_indices,
						band_offsets);
		...
		case SystemLabel.ORG_BIL:
			...
			return new ComponentSampleModel(data_buffer_type, tileWidth, tileHeight, pixel_stride, scanline_stride,
	...
	/***********************************************************************
	 * High-level read function. Follow the See Also link for complete
	 * description
	 * 
	 * @see VicarInput#readTile(int,int,SampleModel,DataBuffer)
	 */
	public void readTile(int x, int y, SampleModel m, DataBuffer b) throws IOException {
	 ...
	 */
	public void readTile(int x, int y, int w, int h, int x_off, int y_off, SampleModel m, DataBuffer b)
			throws IOException {
	 ...
	 */
	public synchronized void readTile(int x, int y, int w, int h, int x_off, int y_off, int bandList[], SampleModel sm,
			DataBuffer db) throws IOException {
		int i;
		int buffer_needed;
		int line, samp;
		int band, band_ind;
		...
		logger.trace("VicarInputFile.readTile " + x + "," + y + " " + w + "x" + h);
		logger.trace(" _system.getNS() " + _system.getNS() + "  _system.getNL() " + _system.getNL()
				+ "  _system.getNB() " + _system.getNB());
		logger.trace(" _system.getN1() " + _system.getN1() + "  _system.getN2() " + _system.getN2()
				+ "  _system.getN3() " + _system.getN3());
		...
		logger.trace(" " + x + "," + y + " " + w + "x" + h);
		logger.trace("  x_off " + x_off + "   y_off " + y_off);

		// Exception e = new Exception("VicarInputFile.readTile");
		// e.printStackTrace();

		if (x_off + w > sm.getWidth())
			throw new ArrayIndexOutOfBoundsException("Illegal width in VICAR readTile: " + w + ", x_off=" + x_off
					+ ", width=" + sm.getWidth());
		if (y_off + h > sm.getHeight())
			throw new ArrayIndexOutOfBoundsException("Illegal height in VICAR readTile: " + h + ", y_off=" + y_off
					+ ", height=" + sm.getHeight());

		PixelAccessor pa = new PixelAccessor(sm, null);
		Rectangle area = new Rectangle(x_off, y_off, w, h);
		UnpackedImageData data = pa.getPixels(Raster.createWritableRaster(sm, db, null), area, db.getDataType(), true);
		...
		case SystemLabel.ORG_BSQ:
			...
			for (band_ind = 0; band_ind < num_bands; band_ind++) {
				...
				case SystemLabel.TYPE_COMP:
					 ...
					 * odds are imaginary. "band_ind" is a simple // index into
					 * the output band array. for (line=0; line < h; line++) {
					 * readRecordCompNS(_float_buffer, x, w, 0, line+y, band/2);
					 * int c = band % 2; // 0=real, 1=imag for (i=0; i < w; i++)
					 * { sm.setSample(x_off+i, y_off+line, band_ind,
					 * _float_buffer[i+c], db); } } break; !!!!
					 */
				}
			} // end band loop
			break;
			...
			for (line = 0; line < h; line++) {
				...
				case SystemLabel.TYPE_HALF:
					for (band_ind = 0; band_ind < num_bands; band_ind++) {
						...
						sdata = data.getShortData(band);
						offset = data.getOffset(band) + line * data.lineStride;
						readRecordNS(sdata, x, w, offset, data.pixelStride, band, line + y);
					}
					break;
				case SystemLabel.TYPE_USHORT:
					for (band_ind = 0; band_ind < num_bands; band_ind++) {
						...
						sdata = data.getShortData(band);
						offset = data.getOffset(band) + line * data.lineStride;
						readRecordUshortNS(sdata, x, w, offset, data.pixelStride, band, line + y);
					}
					break;
				case SystemLabel.TYPE_FULL:
					for (band_ind = 0; band_ind < num_bands; band_ind++) {
						...
						idata = data.getIntData(band);
						offset = data.getOffset(band) + line * data.lineStride;
						readRecordNS(idata, x, w, offset, data.pixelStride, band, line + y);
					}
					break;
				case SystemLabel.TYPE_REAL:
					for (band_ind = 0; band_ind < num_bands; band_ind++) {
						...
						fdata = data.getFloatData(band);
						offset = data.getOffset(band) + line * data.lineStride;
						readRecordNS(fdata, x, w, offset, data.pixelStride, band, line + y);
					}
					break;
				case SystemLabel.TYPE_DOUB:
					for (band_ind = 0; band_ind < num_bands; band_ind++) {
						...
						ddata = data.getDoubleData(band);
						offset = data.getOffset(band) + line * data.lineStride;
						readRecordNS(ddata, x, w, offset, data.pixelStride, band, line + y);
					}
					break;
				case SystemLabel.TYPE_COMP:
					throw new UnsupportedOperationException("readTile() for Complex data not implemented yet!");
					 ...
					 * !!!! // "band" is file_nb*2, where evens are real and //
					 * odds are imaginary. "band_ind" is a simple // index into
					 * the output band array. for (band_ind=0; band_ind <
					 * num_bands; band_ind++) { band = bandList[band_ind];
					 * readRecordCompNS(_float_buffer, x, w, 0, band/2, line+y);
					 * int c = band % 2; // 0=real, 1=imag for (i=0; i < w; i++)
					 * { sm.setSample(x_off+i, y_off+line, band_ind,
					 * _float_buffer[i+c], db); } } break; !!!!
					 */
		...
		case SystemLabel.ORG_BIP:
			...
			case SystemLabel.TYPE_BYTE:
				...
				for (line = 0; line < h; line++) {
					for (samp = 0; samp < w; samp++) {
						...
						for (band_ind = 0; band_ind < num_bands; band_ind++) {
							band = bandList[band_ind];
							bbdata[band_ind][band_offsets[band_ind] + samp * data.pixelStride + line * data.lineStride] = bbuf[band
									- sb];
						}
					}
				}
			...
			case SystemLabel.TYPE_HALF:
				...
				for (line = 0; line < h; line++) {
					for (samp = 0; samp < w; samp++) {
						...
						for (band_ind = 0; band_ind < num_bands; band_ind++) {
							band = bandList[band_ind];
							ssdata[band_ind][band_offsets[band_ind] + samp * data.pixelStride + line * data.lineStride] = sbuf[band
									- sb];
						}
					}
				}
			...
			case SystemLabel.TYPE_USHORT:
				...
				for (line = 0; line < h; line++) {
					for (samp = 0; samp < w; samp++) {
						readRecordUshortNS(ubuf, sb, nb, 0, 1, samp + x, line + y);
						for (band_ind = 0; band_ind < num_bands; band_ind++) {
							band = bandList[band_ind];
							ssdata[band_ind][band_offsets[band_ind] + samp * data.pixelStride + line * data.lineStride] = ubuf[band
									- sb];
						}
					}
				}
			...
			case SystemLabel.TYPE_FULL:
				...
				for (line = 0; line < h; line++) {
					for (samp = 0; samp < w; samp++) {
						...
						for (band_ind = 0; band_ind < num_bands; band_ind++) {
							band = bandList[band_ind];
							iidata[band_ind][band_offsets[band_ind] + samp * data.pixelStride + line * data.lineStride] = ibuf[band
									- sb];
						}
					}
				}
			...
			case SystemLabel.TYPE_REAL:
				...
				for (line = 0; line < h; line++) {
					for (samp = 0; samp < w; samp++) {
						...
						for (band_ind = 0; band_ind < num_bands; band_ind++) {
							band = bandList[band_ind];
							ffdata[band_ind][band_offsets[band_ind] + samp * data.pixelStride + line * data.lineStride] = fbuf[band
									- sb];
						}
					}
				}
			...
			case SystemLabel.TYPE_DOUB:
				...
				for (line = 0; line < h; line++) {
					for (samp = 0; samp < w; samp++) {
						...
						for (band_ind = 0; band_ind < num_bands; band_ind++) {
							band = bandList[band_ind];
							dddata[band_ind][band_offsets[band_ind] + samp * data.pixelStride + line * data.lineStride] = dbuf[band
									- sb];
						}
					}
				}
			...
			case SystemLabel.TYPE_COMP:
				...
				/*
				 * !!!! // "band" is file_nb*2, where evens are real and // odds
				 * are imaginary. "band_ind" is a simple // index into the
				 * output band array. float fb[] = new float[2]; for (samp=0;
				 * samp < w; samp++) { for (band_ind=0; band_ind < num_bands;
				 * band_ind++) { if (bandList == null) band = band_ind; else
				 * band = bandList[band_ind];
				 * 
				 * readRecordCompNS(fb, band/2, 1, 0, samp+x, line+y);
				 * _float_buffer[band_ind] = fb[band%2]; }
				 * sm.setPixel(x_off+samp, y_off+line, _float_buffer, db); }
				 * break; !!!!
				 */
	...
	/***********************************************************************
	 * Internal routine for calculating the position to seek to given the record
	 * address (n2/n3) and the offset within the record. The binary headers and
	 * prefixes are taken into account, i.e. position 0 is the first pixel of
	 * the record (past the prefix), and line/band 0 is the first line/band of
	 * pixel data (past the headers).
	 * 
	 * @throws IOException
	 *             if start, n2 or n3 exceed the bounds of the image
	 */
	protected long calcFilePos(int start, int n2, int n3) throws IOException {
		 ...
		 * logger.trace(" pixelSize="+_system.getPixelSize() );
		 **/

		if (start < 0 || start >= _system.getN1())
			throw new IOException("Attempt to read past edge of image for dimension 1: N1=" + _system.getN1()
					+ ", read position=" + start);
		if (n2 < 0 || n2 >= _system.getN2())
			throw new IOException("Attempt to read past edge of image for dimension 2: N2=" + _system.getN2()
					+ ", read position=" + n2);
		if (n3 < 0 || n3 >= _system.getN3())
			throw new IOException("Attempt to read past edge of image for dimension 3: N3=" + _system.getN3()
					+ ", read position=" + n3);

		return _lblsize_front + (_system.getNLB() + (long) n3 * _system.getN2() + (long) n2) * _system.getRecsize()
				+ (long) start * _system.getPixelSize() + _system.getNBB();
	}

	/***********************************************************************
	 * Internal routine to seek to a given spot in the file. This is trivial for
	 * random-access files, but a bit tricky for sequential ones.
	 * <p>
	...
	protected void seekToLocation(long pos) throws IOException {
		...
		if (pos < _current_file_pos)
			throw new NonSequentialAccessException("Non-sequential access attempted to file, desired pos=" + pos
					+ ", current pos=" + _current_file_pos);
		...
		if (!_random_allowed && pos > _current_file_pos) {
			if (pos - _current_file_pos > Integer.MAX_VALUE)
				throw new IOException("Attempt to skip too many bytes (" + (pos - _current_file_pos)
						+ ") in sequential-only VICAR file");
	 ...
	 * as well as all the other <code>stream_</code>* functions, is required to
	 * handle the plethora of possible input stream objects mandated by the
	 ...
	 * 
	 * @throws UnsupportedOperationException
	 *             if the stream's type is not recognized.
	 */
	...
	protected void stream_read(byte[] b) throws IOException {
		...
		throw new UnsupportedOperationException("Read not supported for this stream type: "
				+ _input_stream.getClass().toString());
	}
