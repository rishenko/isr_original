...
/**
 * This class maintains an entire set (property set, history task, or system) of
 * VICAR labels, with a collection of key/value pairs (Item's).
 * <p>
 * Note that by definition of VICAR labels, the order of items within a set is
 * irrelevant and unspecified, and duplicates are not allowed. This class
 ...
 * <p>
 * This class ensures that History and Property sets (as defined by the set's
 * "flavor") start with the appropriate keywords ("PROPERTY" for property sets,
 * "TASK", "USER", and "DAT_TIM" for history tasks). The values of these can be
 * modified but the items themselves cannot be removed or unset. Note that per
 * VICAR conventions, the property and task names are converted to upper case.
 * <p>
 * The set's "flavor" (system, property, or history) must be set at construction
 * time and cannot be changed (due to the required header labels).
 * <p>
 * This class implements Set so it can be used as a collection. Additional
 * constraints imposed beyond those of Set: all elements must be instances of
 * <code>VicarLabelItem</code>.
 */

public class VicarLabelSet implements Cloneable, Serializable, Set<VicarLabelItem> {

	private static final long serialVersionUID = 7671896637614046097L;
	 ...
	 * @serial
	 */
	protected List<VicarLabelItem> _items;
	...
	/***********************************************************************
	 * Create a set with the given flavor and name. The instance is defaulted to
	 * 0; presumably it will be set immediately by
	...
	/***********************************************************************
	 * Create a set with the given flavor. The instance is defaulted to 0;
	 * presumably it will be set immediately by <code>VicarLabelCategory</code>.
	 */
	...
	/***********************************************************************
	 * No-args constructor mandated by the <code>Collection</code> interface. It
	 * hard-codes the flavor to <code>SYSTEM</code>. For this reason, this
	 ...
	 * constructor w.r.t. flavor... in other words
	 * <em>don't use it this way</em>.
	 * <p>
	 * In any case, all members of the Collection must be a
	 * <code>VicarLabelItem</code>.
	 * 
	 * @throws ClassCastException
	 *             if the provided collection does not contain a
	 *             <code>VicarLabelItem</code>.
	 * @throws IllegalArgumentException
	 *             if an item is not VicarLabelItem or is an inappropriate
	 *             reserved keyword.
	 * @see #VicarLabelSet()
	 * @see #VicarLabelSet(Collection, int)
	 */
	public VicarLabelSet(Collection<VicarLabelItem> c) {
		this(c, (c instanceof VicarLabelSet) ? ((VicarLabelSet) c).getFlavor() : LABEL_SYSTEM);
	}

	/***********************************************************************
	 * Construct a <code>VicarLabelSet</code> from another
	 * <code>Collection</code>, using the specified flavor. This may be used to
	 * change flavors of a <code>VicarLabelSet</code>. The provided Collection
	 * may be an instance of this class, or any other collection. Any reserved
	 * keywords (<code>PROPERTY, TASK, USER, DAT_TIM, LBLSIZE</code>) that
	 * appear in the input collection which are not appropriate for the given
	 * flavor, will cause an exception to be thrown, in order to maintain label
	 * consistency.
	 * <p>
	 * Note that if the special items are not present, defaults will be used, so
	 * the name at least (for property/history) should be set independently.
	 * 
	 * @throws ClassCastException
	 *             if the provided collection does not contain a
	 *             <code>VicarLabelItem</code>.
	 ...
	 */
	public VicarLabelSet(Collection<VicarLabelItem> c, int flavor) {
		this(flavor);
		addAll(c);
	}

	/***********************************************************************
	 * Create a set from an XML Element. See <code>fromXML()</code> for more
	 * information (the arguments are identical). The only difference is, the
	 * flavor is set from the XML (since this is a constructor), while
	 * <code>fromXML()</code> is unable to change the flavor.
	 * <p>
	 * The instance is defaulted to 0; presumably it will be set immediately by
	 * <code>VicarLabelCategory</code>.
	 * <p>
	 * If the supplied node is not &lt;system&gt;, &lt;property&gt;, or
	 * &lt;history&gt;, a warning is issued and &lt;history&gt; is assumed
	 * (being the least "damaging" of the three types for incorrect values).
	 * <p>
	 * 
	 * @see #fromXML(Element, List)
	 */
	public VicarLabelSet(Element set, List<String> errorList) {
		...
		if (tagName.equalsIgnoreCase("system")) {
			...
			if (errorList != null)
				errorList.add("Invalid label set name " + tagName
						+ ".  Requires system, property, or history.  History assumed.");
			_flavor = LABEL_HISTORY;
		}
		_name = null;
		_instance = 0;
	 ...
	 * labels. Note that System labels have no header. If the set name has not
	 * been provided, it is set to "UNSET". The user and dat_tim elements of the
	 * History label are also set to "unset". These values should be overridden
	 * in all cases; they should never appear in a final label if the program is
	 * properly written.
	 */
	protected void initItems() {
		_items = new ArrayList<VicarLabelItem>();

		switch (_flavor) {
		case LABEL_SYSTEM: // nothing special
			break;
		case LABEL_PROPERTY:
			add(new VicarLabelItem("PROPERTY", (_name == null) ? "UNSET" : _name));
			break;
		case LABEL_HISTORY:
			add(new VicarLabelItem("TASK", (_name == null) ? "UNSET" : _name));
	...
	/***********************************************************************
	 * Get the label flavor (LABEL_SYSTEM, LABEL_PROPERTY, LABEL_HISTORY). The
	 * flavor cannot be set other than at construction time.
	 */
	 ...
	 * PROPERTY or TASK headers of property or history labels. If the flavor is
	 * SYSTEM, this call is ignored.
	 * <p>
	...
	/***********************************************************************
	 * Two VicarLabelSet objects are equal for the purposes of this routine if
	 * their Names are equal. The instances are NOT considered. While this is
	 * technically untrue (name and instance should both be equal), there is no
	 * facility to take more than one object as a parameter to equals(), and
	 * doing it this way enables certain searches in VicarLabelCategory. We also
	 * test for equality of the Name with a String.
	 */
	 ...
	 * <p>
	 * The iterator returned is actually an instance of
	 * <code>ListIterator</code>; despite the fact that we extend
	 * <code>Set</code> there is an attempt made to preserve list order (as
	 * stated in the class comments). If <code>ListIterator</code> functions are
	 * required, a cast may be made.
	 */
	public Iterator<VicarLabelItem> iterator() {
	...
	/***********************************************************************
	 * Add an item to the list. If the given keyword matches one already in the
	 * list, the old item is replaced. Otherwise, the item is added to the end.
	 * The reserved names LBLSIZE, PROPERTY, TASK, USER, and DAT_TIM may only be
	 * added to the proper flavor of label (otherwise an
	 * <code>IllegalArgumentException</code> will be thrown).
	 * <p>
	 * If PROPERTY or TASK is set, the value is converted to uppercase, per
	 * VICAR convention.
	 * <p>
	 * <em>Note:</em> This function violates the contract of
	 * <code>Set.add</code> in the following way: if the item already exists
	 * (according to the keyword), the new item <em>replaces</em> the old one.
	 * This modification is needed because item equality is determined only by
	 * the keyword; the value is not checked. Thus it is useful to replace the
	 * existing item, as opposed to a true <code>Set</code> where equality is
	 * defined by the entire object.
	 * 
	 * @return true always
	 * @throws IllegalArgumentException
	 */
	 ...
	 *             but this need not be declared
	 * @throws IllegalArgumentException
	 *             if an attempt is made to delete TASK, USER, DAT_TIM, or
	 *             PROPERTY.
	 */
	public VicarLabelItem removeItem(int index) {
		VicarLabelItem item = (VicarLabelItem) _items.get(index);
		if (item.equals("PROPERTY") || item.equals("TASK") || item.equals("USER") || item.equals("DAT_TIM"))
	...
	/***********************************************************************
	 * Returns a human-readable representation of all items in this set. The
	 * returned string is <em>NOT</em> suitable for directly writing to a label!
	 */
	public String toString() {
		...
		for (VicarLabelItem labelItem : _items) {
			buf.append(labelItem.toString());
			buf.append("\n");
		}
	 ...
	 * <p>
	 * 
	 * @return The first index <em>not</em> put in the string (due to
	 *         overfilling the capacity); i.e. it is the index you would supply
	 *         to <code>start_index</code> in the next call. Returns -1 if all
	 *         items fit.
	 */
	public int toLabelString(StringBuffer buf, int max_length, int start_index) {
		for (int i = start_index; i < size(); i++) {
			String item = getItem(i).toLabelString();
			if ((max_length > 0) && (buf.length() + item.length() + 2 > max_length)) {
				// Exceeded capacity!
				return i;
			}
			buf.append(item);
	 ...
	 * &lt;property&gt;, or &lt;history&gt;). Subnodes will be created for each
	 * item as necessary. See the VICAR label DTD. !!!!TBD: reference to this
	 * DTD!!!!
	 * <p>
	 ...
	 * 
	 * @param document
	 *            The <code>Document</code> which will contain the node.
	 * @throws IllegalArgumentException
	 *             if the label's flavor is unknown.
	...
	public Node toXML(Document doc) {
		...
		for (VicarLabelItem labelItem : _items) {
			...
			if (labelItem.equals("PROPERTY") || labelItem.equals("TASK") || labelItem.equals("USER")
					|| labelItem.equals("DAT_TIM"))
				continue;

			Node node = labelItem.toXML(doc);
	 ...
	 * &lt;property&gt;, or &lt;history&gt;). Items will be created for each
	 * subnode as necessary. See the VICAR label DTD. !!!!TBD: reference to this
	 * DTD!!!!
	 * <p>
	 * Items found in the XML are added to the set. This means that existing
	 * items will be retained in the output if not overridden. (the behavior of
	 * <code>add()</code> is to replace items with the same keyword). If you
	 ...
	 * <p>
	 * If anything is wrong with the supplied node, error/warning messages will
	 * be appended to the supplied <code>errorList</code>. However, the parser
	 * keeps working as long as possible, despite errors. The practical upshot
	 * of this is that the errors can usually be ignored; the parser will do the
	 * best it can. However, the errors can be displayed to the user if desired.
	 * An exception will be thrown only if something unrecoverable happens.
	 * <p>
	 * If the required header attributes are not present (<code>TASK</code>,
	 * <code>DAT_TIM</code>, etc.), the appropriate label items will be
	 * unchanged. This generally means they are set with the value "UNSET" (or
	 * "unset") per <code>initItems()</code>. Reasonable values for these should
	 * be supplied before writing the label. Note that warnings will be issued
	 * via <code>errorList</code> if the attributes are missing.
	 * <p>
	 * 
	 * @param set
	 *            The <code>Element</code> from which to extract the item info.
	 * @param errorList
	 *            A list of <code>String</code>s to which any parsing errors
	 *            will be appended. Can be <code>null</code>, in which case no
	 *            errors are returned.
	 * @throws DOMException
	 *             if something unrecoverable happens. Improperly formatted
	 *             items or violations of the DTD do not necessarily result in
	 *             exceptions; parsing continues as long as possible and
	 *             problems are appended to the <code>errorList</code>.
	 * 
	 * @see VicarLabelItem#fromXML(Element, List)
	 * @see #initItems()
	 */
	public void fromXML(Element set, List<String> errorList) {
		...
		if (tagName.equalsIgnoreCase("system")) {
			...
			if (errorList != null)
				errorList.add("Invalid label set name " + tagName
						+ ".  Requires system, property, or history.  Flavor unchanged.");
			new_flavor = _flavor;
		}

		if (_flavor != new_flavor && errorList != null) {
			errorList.add("Label set flavor cannot be changed to " + tagName + ".  Headers may be incorrect.");
		}
		...
		case LABEL_HISTORY:
			...
			if (value.length() == 0)
				value = null;
			if (value == null && errorList != null)
				errorList.add("History task's user attribute is missing for task " + getItem("TASK"));
			if (value != null)
			...
			if (value.length() == 0)
				value = null;
			if (value == null && errorList != null)
				errorList.add("History task's dat_tim attribute is missing for task " + getItem("TASK"));
			if (value != null)
		...
		for (int i = 0; i < nitems; i++) {
			...
			if (!(item instanceof Element)) {
				if (errorList != null)
					errorList.add(tagName + " label, item #" + i + " is not a DOM Element type");
			} else {
	...
	public Object clone() {
		...
		set._items = (ArrayList<VicarLabelItem>) ((ArrayList<VicarLabelItem>) (set._items)).clone();
		for (int i = 0; i < set._items.size(); i++) {
			VicarLabelItem labelItem = (VicarLabelItem) set._items.get(i).clone();
			set._items.set(i, labelItem);
		}
	...
	/***********************************************************************
	 * Returns true if this set contains no elements. Mandated by
	 * <code>Set</code>.
	 */
	 ...
	 * <code>Set</code>. The Object may be a <code>VicarLabelItem</code> (only
	 * the keyword is matched) or a <code>String</code> (likewise, matches the
	 * keyword).
	 * <p>
	 * In order to allow contains(String), <em>both</em>
	 * <code>o.equals(e)</code> and <code>e.equals(o)</code> are tested. This
	 * slightly contradicts the <code>Set</code> contract for
	 * <code>contains()</code>.
	 */
	public boolean contains(Object o) {
		for (VicarLabelItem labelItem : _items) {
			if (o.equals(labelItem) || labelItem.equals(o))
				return true;
		}
		// TODO kmcabee check: if it doesn't contain it, why return true?
		return true;
	}

	/***********************************************************************
	 * Returns an array containing all of the elements in this set. Mandated by
	 * <code>Set</code>.
	 * 
	 * @see Set#toArray()
	 */
	 ...
	 * type is that of the specified array. Mandated by <code>Set</code>.
	 * 
	 * @see Set#toArray(Object[])
	 */
	@SuppressWarnings("unchecked")
	public VicarLabelItem[] toArray(Object[] a) {
		return (VicarLabelItem[]) _items.toArray(a);
	}

	/***********************************************************************
	 * Removes the specified element from this set if it is present. Mandated by
	 * <code>Set</code>. The item must be a <code>String</code> or
	 * <code>VicarLabelItem</code>. In either case, the keyword is extracted and
	 * removeItem(keyword) is called.
	 * 
	 * @throws IllegalArgumentException
	 *             if a reserved keyword is removed
	 ...
	 * Replaces any items already present as in <code>add()</code>. Mandated by
	 * <code>Set</code>. All items in the provided Collection must be
	 * <code>VicarLabelItem</code>s.
	 */
	public boolean addAll(Collection<? extends VicarLabelItem> c) {
		boolean changed = false;
		// Exceptions are thrown automatically by the cast, and by add()
		for (VicarLabelItem item : c) {
			add(item);
			changed = true;
		}
		return changed;
	...
	/***********************************************************************
	 * Returns the hash code value for this set. The hash code is defined as the
	 * sum of the hash codes of the keywords in order to be consistent with
	...
	public int hashCode() {
		int hash = 0;
		for (VicarLabelItem item : _items) {
