 ...
 * information contained in the label. <br>
 * 
 * @see VicarInputImage
 * @see VicarInput
 */
...
public class ISISInputFile extends VicarInputFile {
	...
	IsisSystemLabel _isisSystem = null; // ISIS and ISIS have the same extra
										// stuff

	// for testing
	private int _readMin, _readMax, _readCt;

	private static final Logger logger = LoggerFactory.getLogger(ISISInputFile.class);
	...
	public ISISInputFile() {
		super();
		logger.debug("%%%%%%% ISISInputFile constructor $$$$$$$$$$$$$$$");
	}
	...
	public ISISInputFile(ImageDecodeParam imageDecodeParam) {
		...
		// if (debug)
		// logger.debug("%%%%%%% ISISInputFile constructor $$$$$ with VicarPdsIsisImageDecodeParam");
		logger.debug("%%%%%%% ISISInputFile constructor $$$$$ with ImageDecodeParam");
	}
	...
	/*
	 * 
	 * openInternal() has been eliminated. setupLabels() does the ISIS specific
	...
	protected void setupLabels() {
		...
		logger.debug("ISISInputFile.setupLabels()");
		logger.debug("input type: " + _input_stream);

		if (_input_stream instanceof ImageInputStream) {

			logger.debug("calling ISISLabelToDOM  instanceof ImageInputStream");
			// ISISLabelToDOM pdsLabel2Dom = new ISISLabelToDOM((DataInput)
			// _input_stream, null);
			ISISLabelToDOM isisLabel2Dom = new ISISLabelToDOM((ImageInputStream) _input_stream, null);
			logger.debug("after calling ISISLabelToDOM");
			_ISIS_document = isisLabel2Dom.getDocument();
			if (logger.isDebugEnabled()) {
				logger.debug("_ISIS_document: " + _ISIS_document);
			...
			// hopefully ISIS counts the embedded Vicar label in its values for
			// label size
			...
			// logger.debug("calling ISISLabelToDOM");
			...
			logger.debug("Improper input type: " + _input_stream);
			logger.debug("can't read the header ");
			return;
		}

		logger.debug("  _lblsize_front " + _lblsize_front);
		...
		logger.debug("  ***** _bsb= " + _bsb + "  _lsb=" + _lsb + "  _ssb=" + _ssb);
	...
	protected void openInternalLast() {
		...
		/**
		 * logger.debug("openInternalLast *************************************"
		 * ); logger.debug("_line_prefix_bytes "+_line_prefix_bytes);
		 * logger.debug("_line_suffix_bytes "+_line_suffix_bytes);
		 * 
	...
	public void setToProperties(Hashtable properties) {
		...
		logger.debug("ISISInputFile.setToProperties $$$$$$$$$$$$$$$$$$$$$$");

		if (properties != null) {
			String n = "ISIS";
			logger.debug("put ImageFormatName " + n);
	...
	public SystemLabel createSystemLabel(Document doc) {
		...
		logger.debug("ISISInputFile.createSystemLabel");
		...
		logger.debug("ISISInputFile.createSystemLabel() " + xPath);
		...
		if (result == null) {
			logger.debug("ISISInputFile.createSystemLabel() incompatable Document");
			return (SystemLabel) null;
		}

		logger.debug("createSystemLabel LOOKING for QUBE ****************");
		node = domUtils.getSingleNode(_ISIS_document, "//OBJECT[@name='QUBE']");
		if (node != null) { // ^QUBE
			// serialize the Node returned to see if it's what was expected

			logger.debug("createSystemLabel for QUBE");
			domUtils.serializeNode(node, "QUBE.xml", "xml");
			logger.debug("serialized to QUBE.xml");
			...
			if (nodeValue != null) {
				...
				logger.debug("AXIS_NAME " + nodeValue);
			...
			logger.debug("CORE_VALID_MINIMUM " + nodeValue);
			core_valid_minimum = nodeValue; // store as a String, figure out how
											// to use the value later
			...
			// values.
			// The USGS documentation states that there are really only three
			// storage
			...
			for (int i = 0; i < axes_names.length; i++) {
				logger.debug("axes=" + axes_names[i] + "  " + core_items[i] + "   suffix=" + suffix_items[i]);
				if (axes_names[i].indexOf("SAMP") >= 0) { // if value at axes[i]
															// resembles "SAMP"
					if (i == 1)
						org = "BIP"; // according to the above, this is unique
										// enough
					ns = core_items[i];
					...
					if (i == 1)
						org = "BIL"; // according to the above, this is unique
										// enough
					nb = core_items[i];
					...
					if (i == 1)
						org = "BSQ"; // according to the above, this is unique
										// enough
					nl = core_items[i];
			...
			// if (debug) logger.debug("ssb "+ssb+"   bsb "+bsb+"   lsb "+lsb);
			// ssb = 0;
			logger.debug("ssb " + ssb + "   bsb " + bsb + "   lsb " + lsb);
			logger.debug("bytes per pixel " + bits + "   CORE__ITEM_TYPE " + core_item_type);
			...
			case 1:
				...
				break;
			case 2:
				// check CORE_VALID_MINIMUM = -32752 to see if this is signed or
				// unsigned
			...
			case 4:
				...
				break;
			default:
				logger.debug("format we don't handle " + bits + " bits yet");
				break;
			...
			node = domUtils.getSingleNode(_ISIS_document, "//OBJECT[@name='IMAGE']");
...
//	        if (debug) System.out.println("LINES_SAMPLES "+nodeValue+" ###############");
	...
	/**
	 * 
	 * This is specific to a Document filled from a ISIS image label calculate
	...
	public int getImageStart(Document doc) {
		...
		String nativeMetadataFormatName = "ISIS_LABEL"; // put this in the
														// ISISCodec ???
		...
		// get the needed data from the result node
		if (result == null) {
			logger.debug("ISISInputFile.getLabelSize() incompatable Document");
			return 0;
		}
		...
		logger.debug("ISISInputFile.getLabelSize() RECORD_BYTES=" + nodeValue);

		// ^QUBE may not be legal XML ???
		xPath = "//item[@key='^QUBE']";
		nodeValue = domUtils.getItemValue(root, xPath);
		imageStart = record_bytes;
		if (!nodeValue.equals("")) {
			qube_starting_record = Integer.parseInt(nodeValue);
			logger.debug("ISISInputFile.getLabelSize() ^QUBE=" + nodeValue);
			imageStart = record_bytes * (qube_starting_record - 1);
			logger.debug("ISISInputFile.getImageStart() imageStart=" + imageStart);
		}
	...
	protected long calcFilePos(int start, int n2, int n3) throws IOException {
		...
		case SystemLabel.ORG_BSQ:
			 ...
			 * (n3 % 100 == 1 && n2 %100 == 1)) {
			 * System.out.print("case SystemLabel.ORG_BSQ  start="
			 * +start+" samp="
			 * +samples+" bands="+bands+" lines="+lines+" s-n2="+n2);
			 * logger.debug
			 * (" l-n3="+n3+" _ssb="+_ssb+" _lsb="+_lsb+" suff="+suffix ); }
			 ***/
		...
		case SystemLabel.ORG_BIP:
			 ...
			 * logger
			 * .debug(" l-n3="+n3+" _ssb="+_ssb+" _bsb="+_bsb+" suff="+suffix );
			 * }
			 **/
		...
		case SystemLabel.ORG_BIL:
			 ...
			 * logger.debug(" _bsb="+_bsb+" bands="+bands+" suffix="+suffix ); }
			 **/
			break;
		}
		 ...
		 * logger.debug(" getRecsize() "+ _system.getRecsize()+
		 * " getPixelSize() "+_system.getPixelSize() ); }
		 ***/
		// logger.debug(" suffix="+suffix+" filePos="+filePos );
		return filePos;
	}
