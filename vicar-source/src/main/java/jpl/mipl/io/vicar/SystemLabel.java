...
/**
 * This class is a bean that maintains all VICAR system label information. Note
 * that no value checking is performed, since there are no easy facilities for
 * validation in the beans system, without going through the VetoableChange
 * mechanism. Besides, we want to accept whatever is in a label, and do the best
 * we can with it.
 * <p>
 * N1, N2, N3, and NS, NL, NB are kept up-to-date with each other via the
 * current Org setting. Changes to one set will change the corresponding item in
 * the other set. Note that Org (Organization) must be set first, because the
 * mapping between N1/2/3 and NS/L/B depends on the organziation and is not
 * reversible once set.
 */
...
public class SystemLabel implements Cloneable {
	...
	/**
	 * This is a completely synthetic property, derived from format, intfmt, and
	 * realfmt labels.
	 */
	 ...
	 * existed before these labels came along. Outputs default to the native
	 * format, Java in this case.
	 */
	...
	protected List<VicarLabelItem> _extraItems;
	...
	/**
	 * Table of system label entries. Note that non-required items don't need a
	 * Valid flag since we have defaults. Also, BUFSIZ is not required, although
	 * it is in the C RTL.
	 */
	public static final LabelEntry _systemList[] = {
			...
			new LabelEntry("HOST", LabelEntry.TYPE_STRING, "host", "hostValid", false),
			new LabelEntry("INTFMT", LabelEntry.TYPE_STRING, "intFmt", "intFmtValid", false),
			new LabelEntry("REALFMT", LabelEntry.TYPE_STRING, "realFmt", "realFmtValid", false),
	...
	public SystemLabel() {
		setDefaults();
		_extraItems = new ArrayList<VicarLabelItem>();
	}

	/***********************************************************************
	 * Create a SystemLabel and fill it up.
	 */
	public SystemLabel(VicarLabelSet set) throws IntrospectionException, InvocationTargetException,
	 ...
	 */
	public void readLabel(VicarLabelSet set) throws IntrospectionException, InvocationTargetException,
			IllegalAccessException {
	 ...
	 */
	public void writeLabel(VicarLabelSet set) throws IntrospectionException, InvocationTargetException,
			IllegalAccessException {
	 ...
	 */
	public List<VicarLabelItem> getExtraItems() {
		return _extraItems;
	}

	/***********************************************************************
	 * Return a string representation of the system label. To make this easy, we
	 * convert back to a <code>VicarLabelSet</code> and use its toString().
	 */
	 ...
	 * <li>COMP: complex number, composed of two REALs in the order
	 * (real,imaginary)
	 * </ul>
	 * The following value is accepted, although it is not a valid VICAR data
	 * type. However, other file formats which use this package can find it
	 * useful.
	 * <ul>
	 ...
	 * </ul>
	 * The Set method will convert the three obsolete types to their
	 * counterparts.
	 */
	...
	public void setFormat(String s) {
		...
		if (_format.equals("WORD"))
			_format = "HALF";
		if (_format.equals("LONG"))
			_format = "FULL";
		if (_format.equals("COMPLEX"))
			_format = "COMP";

		_format_code = TYPE_BYTE;
		if (_format.equals("HALF"))
			_format_code = TYPE_HALF;
		if (_format.equals("FULL"))
			_format_code = TYPE_FULL;
		if (_format.equals("REAL"))
			_format_code = TYPE_REAL;
		if (_format.equals("DOUB"))
			_format_code = TYPE_DOUB;
		if (_format.equals("COMP"))
			_format_code = TYPE_COMP;
		if (_format.equals("USHORT"))
			_format_code = TYPE_USHORT;
	 ...
	 * but it is no longer used. It still must be present for historical
	 * reasons, however. BUFSIZ is automatically set the same as RECSIZE,
	 * whenever RECSIZE changes. If you want a certain value for this field for
	 * some reason, set it last. Unlike the C RTL, the Java version does not
	 * require this field... but it must always be present for VICAR
	 * compatibility.
	 * <p>
	...
	/***********************************************************************
	 * Property <code>Dim</code>: The number of dimensions in the file, which is
	 * always equal to 3. Some older images may have a DIM of 2, in which case
	 * some labels will not be present. Note that the dimension is 3 even if
	 * N3=1 (e.g. there is only one band in a BSQ file).
	 */
	public int getDim() {
		return _dim;
	}
	 ...
	 * pixel type) and the INTFMT or REALFMT (for the host representation)
	 * labels.
	 */
	 ...
	 * values are:
	 * <ul>
	 * <li>BSQ: Band SeQuential. The file is a sequence of bands. Each band is
	 * made up of lines, which are in turn made up of samples. N1=Samples,
	 * N2=Lines, and N3=Bands. This is the most common case.
	 * <li>BIL: Band Interleaved by Line. The file is a sequence of lines. Each
	 * line is made up of bands, which are in turn made up of samples.
	 * N1=Samples, N2=Bands, N3=Lines.
	 * <li>BIP: Band Interleaved by Pixel. The file is a sequence of lines. Each
	 * line is made up of samples, which are in turn made up of bands. N1=Bands,
	 * N2=Samples, N3=Lines.
	 * </ul>
	...
	/***********************************************************************
	 * Property <code>NL</code>: The number of lines in the image (same as N2
	 * for BSQ or N3 for BIL and BIP).
	 * <p>
	...
	/***********************************************************************
	 * Property <code>NS</code>: The number of samples in the image (same as N1
	 * for BSQ and BIL or N2 for BIP).
	 * <p>
	 * Setting this property will affect <code>N1</code>, <code>N2</code>, or
	 * <code>N3</code>, depending on the current <code>Org</code> setting.
	 * <code>Org</code> must be set first.
	...
	/***********************************************************************
	 * Property <code>NB</code>: The number of bands in the image (same as N3
	 * for BSQ, N2 for BIL, or N1 for BIP).
	 * <p>
	 * Setting this property will affect <code>N1</code>, <code>N2</code>, or
	 * <code>N3</code>, depending on the current <code>Org</code> setting.
	 * <code>Org</code> must be set first.
	...
	/***********************************************************************
	 * Property <code>N1</code>: The size (in pixels) of the first
	 * (fastest-varying) dimension. Defaults to <code>NS</code> or
	 * <code>NB</code> if not present or 0.
	 * 
	 * <p>
	 ...
	 * dimension. Defaults to <code>NL</code> or <code>NB</code> if not present
	 * or 0.
	 * <p>
	 * Setting this property to a non-0 value will affect <code>NS</code>,
	 * <code>NL</code>, or <code>NB</code>, depending on the current
	 ...
	 * third dimension. The size of the binary header in bytes is given by NLB *
	 * RECSIZE, since NLB is a line count. NLB defaults to 0. Note that the
	 * binary header also includes space reserved for the binary prefix (NBB),
	 ...
	 * be kept consistent with INTFMT and REALFMT. HOST defaults to VAX-VMS. The
	 * value may be anything, as new computer types are occasionally added, but
	 * as of this writing, the possible values are:
	 * <ul>
	 ...
	 * <li>DECSTATN*: DECstation (any DEC MIPS-based RISC machine) running
	 * Ultrix
	 ...
	 * <li>MAC-MPW*: Macintosh running native mode with Mac Programmers
	 * Workbench
	 ...
	 * <p>
	 * Note that <em>ALL</em> Java platforms use the host name "JAVA",
	...
	public void setHostValid(boolean b) {
		_host_valid = b;
	}
	 ...
	 * should be present even if the pixels are a floating-point type. The valid
	 * values are:
	 * <ul>
	 * <li>LOW: Low byte first, "little endian". Used for hosts VAX-VMS,
	 * AXP-VMS, X86-SOLR, DECSTATN, AXP-UNIX, AXP-LINUX, X86-LINUX.
	...
	public boolean isIntFmtValid() {
		return _intfmt_valid;
	}

	public void setIntFmtValid(boolean b) {
		_intfmt_valid = b;
	}

	/***********************************************************************
	 * Property <code>RealFmt</code>: The format used to represent
	 * floating-point pixels (REAL, DOUB, and COMP) in the file. Defaults to
	 * VAX. Note that REALFMT should be present even if the pixels are an
	 * integral type. The valid values are:
	 * <ul>
	...
	public boolean isRealFmtValid() {
		return _realfmt_valid;
	}

	public void setRealFmtValid(boolean b) {
		_realfmt_valid = b;
	}
	...
	public void setBHost(String s) {
		_bhost = s.toUpperCase();
	}
	 ...
	 * INTFMT. The reason BINTFMT is separate is that the data in the binary
	 * label may be in a different host representation than the pixels.
	 * 
	 * @see #getIntFmt()
	 */
	...
	public void setBIntFmt(String s) {
		_bintfmt = s.toUpperCase();
	}

	/***********************************************************************
	 * Property <code>BRealFmt</code>: The format used to represent
	 * floating-point data in the binary label. It can take the same values with
	 * the same meanings as REALFMT. The reason BINTFMT is separate is that the
	 * data in the binary label may be in a different host representation than
	 * the pixels.
	 * 
	 * @see #getRealFmt()
	 */
	...
	public void setBRealFmt(String s) {
		_brealfmt = s.toUpperCase();
	}

	/***********************************************************************
	 * Property <code>BLType</code>: The type of the binary label. This is not a
	 * data type, but is a string identifying the kind of binary label in the
	 * file. It is used for documentation, and so application programs can
	 * process the binary label correctly, without having to be told what kind
	 * it is. It defaults to a null string. The valid values are to be
	 * maintained in a name registry, which will document the actual data layout
	 * for each BLTYPE.
	 */
	...
	public void setBLType(String s) {
		_bltype = s.toUpperCase();
	}
	...
	/***********************************************************************
	 * Creates a deep copy of the label bean. Nothing can be done to the
	 * returned copy that in any way affects the original. This allows the bean
	 * to be returned to the application from the I/O system, without worrying
	 * about the application messing up some internal state.
	 */
	...
	public Object clone() {
		...
		lbl._extraItems = (ArrayList<VicarLabelItem>) ((ArrayList<VicarLabelItem>) (lbl._extraItems)).clone();
		for (int i = 0; i < lbl._extraItems.size(); i++) {
			VicarLabelItem label = (VicarLabelItem) lbl._extraItems.get(i).clone();
			lbl._extraItems.set(i, label);
		}
