 ...
 * <p>
 * All accesses to the PDS file are thread-safe, assuming that nobody else tries
 * to access the underlying stream directly. Thus, multiple threads can issue
 * simultaneous <code>readRecord()</code> or <code>readTile()</code> requests,
 * although each request is handled one at a time via synchronization on the
 * <code>PDSInputFile</code> object. However, if you have a sequential-only
 * stream, all accesses must still be strictly sequential... meaning that the
 * use of multiple threads with sequential streams will not work (the request
 * order would be non-deterministic). For random-hard streams, threads will work
 * but could cause performance hits depending on the ordering of the requests.
 * Random-easy streams should be fine.
 * <p>
 * This reader is capable of reading embedded Vicar labels. If one is
 * encountered a VicarLabel Object will be created and filled. This Object will
 * be placed in properties as "vicar_label" so other programs may access the
 * information contained in the label.
 * <p>
 * Currently only supports 8 and 16 bit image types, and 32 bit IEEE_REAL <br>
 * 
 * @see VicarInputImage
 * @see VicarInput
 */
...
public class PDSInputFile extends VicarInputFile {
	...
	IsisSystemLabel _isisSystem = null; // PDS and ISIS have the same extra
										// stuff
	...
	boolean goNative = false; // this will be set to true if the OAL C native
								// library can
	// be loaded. The flag will then be used to determine if native imnterface
	// calls
	// should be used.

	OaImageKeywords oaImageKeywords = null;

	Logger logger = LoggerFactory.getLogger(PDSInputFile.class);
	...
	public PDSInputFile(ImageDecodeParam imageDecodeParam) {
		...
		// Not now because other changes to that class will be implemented at
		// the same time
		...
		// if (debug)
		// System.out.println("%%%%%%% PDSInputFile constructor $$$$$ with VicarPdsIsisImageDecodeParam");
		if (debug)
	...
	public synchronized void open(String fn) throws IOException {
		filename = fn;
		if (debug)
			System.out.println("PDSInputFile.open(" + fn + ")");
	 ...
	 */
	/**
	 * protected void openInternal() throws IOException { // Make sure we're at
	 * the beginning of the file/stream. // Only does anything if random-access.
	 * seekToLocation(0);
	 * 
	 * _lblsize_front = 0; _lblsize_eol = 0;
	 *********/

	/*
	 * 
	 * openInternal() has been eliminated. setupLabels() does the PDS specific
	 * part and is called from openInternal() in VicarInputFile
	 */
	...
	protected void setupLabels() {
		...
		if (_input_stream instanceof ImageInputStream) {
			...
			// PDSLabelToDOM pdsLabel2Dom = new PDSLabelToDOM((DataInput)
			// _input_stream, null);
			...
			// domUtils.serializeDocument( _PDS_document, "PDS_file.xml",
			// "xml");
			...
			// probably should be done in native eventually OR createSystemLabel
			// will call native
			 ...
			 * setProperty("ImageDecodeParam_ClassName", (Object)
			 * imageDecodeParam.getClass().getName() );
			 * setProperty("ImageFormatName", (Object) "PDS");
			 * setProperty("SystemLabel", (Object) _system);
			 * setProperty("SystemLabel_ClassName", (Object)
			 * _system.getClass().getName() );
			 ***/
			// getLabelSize assumes a PDS compliant label
			// hopefully PDS counts the embedded Vicar label in its values for
			// label size
			...
			// InputStreamReader((DataInput) _input_stream));
		...
		if (debug)
			System.out.println("PDSInputFile.setupLabels() new DOMutils *************************");
		...
		if (value == null) {
			...
			if (debug)
				System.out.println("openInternal recordBytes " + recordBytes + "  " + value);
		} else {
		...
		if (debug)
			System.out.println("  find single node for ^IMAGE_HEADER " + node);
		if (node != null) {
			if (debug)
			...
			// value may be in the attributes also -- above method finds the
			// value in either spot
			if (value == null || value == "") {
				...
				while ((n = ni.nextNode()) != null) {
					...
					if (debug)
						System.out.println("^IMAGE_HEADER [" + ii + "]  subitemValue=>" + subitemValue + "<");
				...
				if (vLen == 0) {
					...
					// we should never get here since there wouldn't be a
					// subitem
					...
					if (debug)
						System.out
								.println("^IMAGE_HEADER  detatchedFilename=" + detatchedFilename + "  value=" + value);
				} else {
					if (debug)
						System.out.println("^IMAGE_HEADER  has " + vLen + " subitems !");
				}

				if (debug)
					System.out.println("^IMAGE_HEADER   v=" + value + "<");
			}

			if (value != null && value != "") {
				if (value.indexOf("<BYTES>") != -1) {
					if (debug)
						System.out.println("<BYTES>   v=" + value + "~");
					String v = value.replaceAll("<BYTES>", ""); // remove
																// <BYTES>
					v = v.trim();
					if (debug)
						System.out.println("<BYTES>   v=" + v + "~");
					_embedded_label_start = Integer.parseInt(v);
					_embedded_label_start--;
				} else if (value.indexOf("<RECORDS>") != -1) {
					String v = value.replaceAll("<RECORDS>", ""); // remove
																	// <RECORD>
					v = v.trim();
					...
					_embedded_label_start = (imageHeader - 1) * recordBytes;
					if (debug)
						System.out.println("imageHeader=" + imageHeader + " _embedded_label_start="
								+ _embedded_label_start);
				}
				_hasEmbeddedVicarLabel = true;
			}
		...
		// pdsLabel2Dom should leave _input_stream at the beginning of the
		// VicarLabel !!!
		...
		_image_size_bytes = (_system.getNLB() + (_system.getN2() * _system.getN3())) * _system.getRecsize();
	 ...
	 * readPdsLabelString
	 * 
	 * read from the stream and get a String containing the PDS label This
	 * method assumes the PDS label's end is signaled by "END"
	 */
	...
	public void readEmbeddedVicarLabel() {
		if (_hasEmbeddedVicarLabel) {
			...
			// _embedded_lblsize_front = _label.readLabelChunk((InputStream)
			// _input_stream);

			if (debug)
				System.out.println("PDSInputFile.openInternal calling readLabelChunk(Object)" + _input_stream);

			try {
				if (debug)
					System.out.println(" * seek to: _embedded_label_start " + _embedded_label_start);
				if (debug)
				...
				if (debug)
					System.out.println(" ******** PDSInputFile.openInternal  read vicar label "
	...
	protected void openInternal() throws IOException {
		...
		if (goNative) {
			...
			if (debug)
				System.out.println("detatchedLabel=" + detatchedLabel + "  detatchedFilename=" + detatchedFilename);

			// Get the host and data formats and set up the VicarDataFormat
			// object.
			...
			if (detatchedLabel == true) {
				...
				RandomAccessFile raf = new RandomAccessFile(detatchedFilename, "r");
			...
			if (_hasEmbeddedVicarLabel) {
				// we have switched _input_stream (if this is a detatched label)
				// to the one which contains
	...
	protected void openInternalLast() {
		...
		readLinePrefixData(_line_prefix_bytes); // this is a Vicar and PDS only
												// operation
	 ...
	 * 
	 * @param Properties
	 *            Object intended to be called with the properties of the
	 *            RenderedImage so some usefull things are put in the images
	 *            properies. The Application which receives this image can use
	 *            these properies. This doesn't seem to work as expected. Some
	 *            Objects seem to go out os scope and become null.
	...
	public void setToProperties(Hashtable properties) {
		...
		if (properties != null) {
			String n = "PDS";
			if (debug)
				System.out.println("put ImageFormatName " + n);
			...
			if (_hasEmbeddedVicarLabel) {
				...
				// putting the label into the images properties allows us to
				// gain access to the
	...
	public SystemLabel createSystemLabel(Document doc) {
		...
		// result = XPathAPI.selectSingleNode(root,
		// PDSMetadata.nativeMetadataFormatName);
		...
		if (debug)
			System.out.println("PDSInputFile.createSystemLabel() " + xPath);
		...
		/*******************************************
		 * determine what this file contains 3 current possibilities are handled
		 * ^IMAGE ^QUBE ^SPECTRAL_QUBE each of the above has an OBJECT associted
		 * with it to describe how it is stored in this file NON-handled
		 * possibilities include ^HISTORY AND ^TABLE
		 *********************************************/

		/*
		 * SystemLabel items which must be set to read a file we must set these.
		...
		/*
		 * grabbing the node first and then trying to get things from that node
		 * does NOT seem to work. Instead we must give a more specific XPATH
		 * expression
		 */
		if (node != null) { // ^IMAGE
			...
			if (debug)
				System.out.println("SAMPLE_BITS <" + nodeValue + "> ");
			...
			nodeValue = domUtils.getItemValue(node, xPath);
			if (debug)
				System.out.println("SAMPLE_TYPE <" + nodeValue + "> ");
			...
			if (nodeValue.equalsIgnoreCase("PC_REAL") && bits == 32) {
				...
				unsignedFlag = false;
				if (debug) {
					System.out.println("PC_REAL REAL");
				}
			}
			...
			if (nodeValue.equalsIgnoreCase("PC_REAL") && bits == 64) {
				...
				unsignedFlag = false;
				if (debug) {
					System.out.println("PC_REAL DOUB");
				}
			}
			...
			if (nodeValue.equalsIgnoreCase("MSB_UNSIGNED_INTEGER") || nodeValue.equalsIgnoreCase("PC_UNSIGNED_INTEGER")
			...
			// what about other bits and sample types ??? I seem to have ignored
			// this
			// another To-do
			// USHORT was added to support PDS, vicar doesn't have USHORT - HALF
			// is signed
			...
			nodeValue = domUtils.getItemValue(node, xPath);
			if (debug)
				System.out.println("LINES_SAMPLES " + nodeValue + " ###############");
			if (nodeValue.equalsIgnoreCase(na) || nodeValue.equalsIgnoreCase(na2)) {
				// do nothing
			} else {
			...
			if (debug)
				System.out.println("PDSInputFile BANDS <" + nodeValue + "> ");
			if (nodeValue != null && nodeValue != "" && nodeValue.equalsIgnoreCase(na) != true
					&& nodeValue.equalsIgnoreCase(na2) != true) { // defaults to
																	// 1
				if (debug)
					System.out.println("PDSInputFile BANDS <" + nodeValue + "> " + nodeValue.length());
			...
			nodeValue = domUtils.getItemValue(node, xPath);
			if (debug)
				System.out.println("LINE_PREFIX_BYTES <" + nodeValue + "> ");
			...
			nodeValue = domUtils.getItemValue(node, xPath);
			if (debug)
				System.out.println("LINE_SUFFIX_BYTES <" + nodeValue + "> ");
			if (nodeValue != null && nodeValue != "") {

				line_suffix_bytes = Integer.parseInt(nodeValue);
			}

			// BAND_STORAGE_TYPE - BAND_SEQUENTIAL LINE_INTERLEAVED
			// SAMPLE_INTERLEAVED
			// BAND_SEQUENCE
			if (nb > 1) {
		...
		} else { // Be sure to search for SPECTRAL_QUBE before QUBE so we don't
					// get a false match
			if (debug)
			...
			if (node != null) { // ^SPECTRAL_QUBE
				...
				if (debug)
					domUtils.serializeNode(node, "SPECTRAL_QUBE.xml", "xml");
				...
				/***
				 * String[] getItemStringArray ( String array ) int[]
				 * getItemIntArray ( String array ) double[] getItemDoubleArray
				 * ( String array )
				 ****/
				// get an array of values from the returned string

				// Determine the storage order from the AXIS_NAME and CORE_ITEMS
				// values.
				// The USGS documentation states that there are really only
				// three storage
				...
				for (int i = 0; i < axes_names.length; i++) {
					if (debug)
						System.out.println("axes=" + axes_names[i] + "  " + core_items[i] + "   suffix="
								+ suffix_items[i]);
					if (axes_names[i].indexOf("SAMP") >= 0) { // if value at
																// axes[i]
																// resembles
																// "SAMP"
						if (i == 1)
							org = "BIP"; // according to the above, this is
											// unique enough
						ns = core_items[i];
						...
						if (i == 1)
							org = "BIL"; // according to the above, this is
											// unique enough
						nb = core_items[i];
						...
						if (i == 1)
							org = "BSQ"; // according to the above, this is
											// unique enough
						nl = core_items[i];
				...
				case 1:
					...
					break;
				case 2:
					// check CORE_VALID_MINIMUM = -32752 to see if this is
					// signed or unsigned
					if (core_valid_minimum < 0) {
						format = "HALF"; // HALF is signed // this is the Vicar
											// name
				...
				default:
					if (debug)
						System.out.println("format we don't handle " + bits + " bits yet");
					break;
				...
				/*
				 * this is a placeholder since I have never received a file with
				 * a ^QUBE in it. It may be exacly the same as a ^SPECTRAL_QUBE
				 */
				if (debug)
					System.out.println("createSystemLabel for ^QUBE");
				node = domUtils.getSingleNode(_PDS_document, "//OBJECT[@name='QUBE']");
				if (node != null) { // ^SPECTRAL_QUBE
					// serialize the Node returned to see if it's what was
					// expected
					if (debug) {
		...
		// USHORT was added to support PDS, vicar doesn't have USHORT - HALF is
		// signed
		sys.setFormat(format); // BYTE HALF FULL REAL DOUB COMP USHORT
		...
		/*
		 * maybees since PDS is now including some ISIS keywords we need to
		 * merge the ISIS and PDSsystemLabel to include CORE_BASE
		 * CORE_MULTIPLIER and SUFFIX's
		 * 
		 * setN1(int n1) fastest varying dimension defaults - don't need to set
	...
	/***************************************************************
	 * convert a text item which is a list to an array of the values in the list
	 * Assumes the item may have () parens around it and the list is comma
	 * seperated allow for space in the items but don't copy the spaces
	 ***********/
	private String[] getItemStringArray(String array) {
		logger.debug("getItemStringArray {}", array);
		if (array == null || array.indexOf("(") == -1 || array.indexOf(")") == -1)
			return new String[] {};
	...
	private int[] getItemIntArray(String array) {
		logger.debug("getItemIntArray {}", array);
		if (array == null || array.indexOf("(") == -1 || array.indexOf(")") == -1)
			return new int[] {};
	...
	private double[] getItemDoubleArray(String array) {
		logger.debug("getItemDoubleArray {}", array);
		if (array == null || array.indexOf("(") == -1 || array.indexOf(")") == -1)
			return new double[] {};
		int start = array.indexOf("(");
		int end = array.indexOf(")");
	...
	/**
	 * Function hasEmbeddedVicarLabel().
	 * <p>
	 * 
	 * <pre>
	...
	/**
	 * 
	 * This is specific to a Document filled from a PDS image label calculate
	 * the size of the image's label in bytes. This is how much must be skipped
	 * to start reading image data. This method use the XPathAPI to search a
	 * Document for a Node. if we get back a node then we extract a value.
	 **/
	...
	public int getImageStart(Document doc) {
		...
		String nativeMetadataFormatName = "PDS_LABEL"; // put this in the
														// PDSCodec ???
		...
		if (node != null) {
			...
			if (value != null && value != "") {
				...
				if (commaPos != -1) {
					/*
					 * String[] v = value.split(","); // JDK1.4 String now has a
					 * regex split method value = v[1]; value =
					 * value.replaceAll(")",""); value = value.trim()
					 */
					...
					// this value might aslso be like the one below and contain
					// <BYTES> in it
				...
				while ((n = ni.nextNode()) != null) {
					...
					subitemValue = domUtils.getNodeValue(n);
					if (debug)
						System.out.println("^IMAGE [" + ii + "]  subitemValue=>" + subitemValue + "<");
					imageSubitemVec.add(subitemValue);
					ii++;
				}
				int vLen = imageSubitemVec.size();
				...
				if (vLen == 0) {
					...
					value = (String) imageSubitemVec.get(1);
					if (debug)
						System.out.println("^IMAGE  detatchedFilename=" + detatchedFilename + "  value=" + value);
				} else {
					if (debug)
						System.out.println("^IMAGE   has " + vLen + " subitems !");
				}
			...
			if (value.indexOf("<BYTES>") != -1) {
				...
				imageStart--; // header value is really the first byte of data,
								// not the amount to skip to begin reading
				if (debug)
					System.out.println("<BYTES>   imageStart=" + imageStart);
			} else if (value.indexOf("<RECORDS>") != -1) {
				String v = value.replaceAll("<RECORDS>", ""); // remove <RECORD>
				if (debug)
					System.out.println("<RECORDS>   v=" + v + "~");
				v = v.trim();
		...
		if (attrs != null) {
			...
			_hasEmbeddedVicarLabel = false;
			if (debug)
				System.out.println("  find single node for ^SPECTRAL_QUBE " + node);
			if (node != null) {
				if (debug)
				...
				_hasEmbeddedVicarLabel = false;
				if (debug)
					System.out.println("  find single node for ^QUBE " + node);
				if (node != null) {
					if (debug)
						System.out.println("   getImageStart() getting value ");
					value = domUtils.getNodeValue(node);
					if (debug)
						System.out.println("   value " + value);
					// value may be in the attributes also -- above method finds
					// the value in either spot
					if (value != null) {
						image_starting_record = Integer.parseInt(value);
					}
					imageStart = (image_starting_record - 1) * record_bytes;
	...
	protected long calcFilePos(int start, int n2, int n3) throws IOException {
		if (debug) {
			 ...
			 * System.out.print(" NBB="+_system.getNBB()+" _line_prefix_bytes="
			 * +_line_prefix_bytes);
		...
		long filePos = _lblsize_front + (_system.getNLB() + (long) n3 * _system.getN2() + (long) n2)
				* _system.getRecsize() + (long) start * _system.getPixelSize() + _system.getNBB();
		...
		case SystemLabel.ORG_BIP:
			...
			/**
			 * if ( n2 == 0 || n2 == 299 || (n3 % 100 == 0 && n2 %100 == 0) ||
			 * (n3 % 100 == 1 && n2 %100 == 1)) {
			 * System.out.print("BIP start="+start
			 * +" samp="+samples+" bands="+bands+" lines="+lines+" s-n2="+n2);
			 * System
			 * .out.println(" l-n3="+n3+" _ssb="+_ssb+" _bsb="+_bsb+" suff="
			 * +suffix ); }
			 **/
		...
		case SystemLabel.ORG_BIL:
			 ...
			 * System.out.print("BIL start="
			 * +start+" lines="+lines+" n2="+n2+" n3="+n3+" _ssb="+_ssb );
			 * System
			 * .out.println(" _bsb="+_bsb+" bands="+bands+" suffix="+suffix ); }
			 **/
			break;
		}
		...
		/***
		 * if ( n2 == 0 || n2 == 299 || (n3 % 100 == 0 && n2 %100 == 0) || (n3 %
		 * 100 == 1 && n2 %100 == 1)) {
		 * System.out.print("  n2="+n2+" n3="+n3+"       filePos "+filePos );
		 * System.out.println(" getRecsize() "+ _system.getRecsize()+
		 * " getPixelSize() "+_system.getPixelSize() ); }
		 ***/
		// System.out.print(" NBB="+_system.getNBB()+" _line_prefix_bytes="
		// +_line_prefix_bytes);
	...
	// throws IOException
	{
		 ...
		 * The second condition is redundant, but just in case... if
		 * (!_label.isReadComplete() && _system.getEOL() != 0) {
		 * seekToLocation(_lblsize_front + _image_size_bytes); // _lblsize_eol =
		 * _label.readLabelChunk((InputStream) _input_stream); _lblsize_eol =
		 * _label.readLabelChunk((Object) _input_stream);
		 * 
		 * // We do not seek back because of sequential or random-hard files. //
		 * The next file access will go wherever it needs to; let's // minimize
		 * work here. } _label.setReadComplete(true);
		 * 
