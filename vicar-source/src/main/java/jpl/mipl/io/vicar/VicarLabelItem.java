...
// just to make javadoc happy

/**
 * This class maintains the key/value pair for a single VICAR label item, and
 * provides mechanisms to retrieve and modify the key and value.
 * <p>
 * Items are stored internally as strings in the format required for the label
 * (with one exception, see below)... but this fact is transparent to the
 * caller, who can use the int/float/double get/set methods as appropriate. The
 * exception is strings... the VICAR standard specifies that single quotes
 * embedded in a string are doubled when written to the file. However, strings
 * are stored internally with the doubles removed (which are added again in
 * <code>toLabelString</code>). This is a slight inconsistency of design, but
 * should be transparent to the caller. Note that <code>toString</code> itself
 * converts to a more human-readable format; <code>toLabelString</code> converts
 * to a format suitable for the label.
 * <p>
...
public class VicarLabelItem implements Cloneable, Serializable {
	private static final long serialVersionUID = 283086072260561414L;
	...
	public VicarLabelItem(String key, String[] value) {
		_key = key.toUpperCase();
		_value = new ArrayList<String>(value.length);
	...
	/***********************************************************************
	 * Create a label item from a single string, but we don't necessarily know
	 * the type, so "guess" at it. This form is indended for use by the label
	 * reader, not by applications (who should know their type!)
	 * 
	 * @param quote
	 *            <code>true</code> if a quote was present (forces a string),
	 ...
	 * @see #VicarLabelItem(String, String, boolean)
	 */
	public VicarLabelItem(String key, List<String> value, boolean quote) {
		_key = key.toUpperCase();
		_value = new ArrayList<String>(value.size());
		if (quote)
			_type = TYPE_STRING;
		else
			_type = TYPE_UNSPECIFIED;
	...
	/***********************************************************************
	 * Create a label item from an XML Element. See <code>fromXML()</code> for
	 * more information (the arguments are identical).
	 * 
	 * @see #fromXML(Element, List)
	 */
	public VicarLabelItem(Element item, List<String> errorList) {
		this();
	...
	/***********************************************************************
	 * Get the "natural" type. Will return TYPE_UNSPECIFIED only if there are no
	 * values. There is no <code>setType</code> because that must be consistent
	...
	/***********************************************************************
	 * Two VicarLabelItem objects are equal if their Keywords are equal. The
	 * values are not considered. This makes searches in VicarLabelSet much
	 * easier. We also test for equality of the keyword with a String.
	 */
	...
	/***********************************************************************
	 * Get the (first) value as an integer. If the type is not TYPE_INTEGER, 0
	 * is returned. Useful if there's only one value.
	 * 
	 * @throws ArrayIndexOutOfBoundsException
	 *             but this need not be declared
	 */
	...
	/***********************************************************************
	 * Get the given element as an integer. If the type is not TYPE_INTEGER, 0
	 * is returned.
	 * 
	 * @throws ArrayIndexOutOfBoundsException
	 *             but this need not be declared
	...
	/***********************************************************************
	 * Get the value as an integer array. If the type is not TYPE_INTEGER, an
	 * array full of 0's is returned.
	 */
	...
	/***********************************************************************
	 * Set the value from a string. When done, the vector will have one and only
	 * one element in it (any previous elements are discarded).
	 */
	...
	/***********************************************************************
	 * Set the value from a float. When done, the vector will have one and only
	 * one element in it (any previous elements are discarded).
	 */
	...
	/***********************************************************************
	 * Set the value from a double. When done, the vector will have one and only
	 * one element in it (any previous elements are discarded).
	 */
	...
	/***********************************************************************
	 * Set the entire value to an integer array. Any previous items are
	 * discarded.
	 */
	public void setValue(int[] value) {
		_value = new ArrayList<String>(value.length);
	...
	/***********************************************************************
	 * Inserts a string at the given index. The new element appears before the
	 * element currently at that index.
	 * 
	 * @throws ArrayIndexOutOfBoundsException
	 *             but this need not be declared
	...
	/***********************************************************************
	 * Inserts an integer at the given index. The new element appears before the
	 * element currently at that index.
	 * 
	 * @throws ArrayIndexOutOfBoundsException
	 *             but this need not be declared
	...
	/***********************************************************************
	 * Inserts a float at the given index. The new element appears before the
	 * element currently at that index.
	 * 
	 * @throws ArrayIndexOutOfBoundsException
	 *             but this need not be declared
	...
	/***********************************************************************
	 * Inserts a double at the given index. The new element appears before the
	 * element currently at that index.
	 * 
	 * @throws ArrayIndexOutOfBoundsException
	 *             but this need not be declared
	...
	/***********************************************************************
	 * Returns the entire Item as a valid VICAR label string, ready to be added
	 * to a label, in key=value form. Specifically, quotes internal to strings
	 * are doubled, per the VICAR standard. (This is not necessary to do for
	 * input because <code>VicarLabelParser</code> handles it).
	 */
	 ...
	 * <code>toLabelString</code> for that. The differences: internal quotes in
	 * strings are not doubled, and spaces are added for readability.
	 * 
	 * @see #toLabelString()
	 */
	 ...
	 * &lt;item&gt; node. See the VICAR label DTD. !!!!TBD: reference to this
	 * DTD!!!!
	 * <p>
	 ...
	 * An exception will be thrown only if something unrecoverable happens. A
	 * missing keyword will default to "UNKNOWN".
	 * 
	 * @param item
	 ...
	 */
	public void fromXML(Element item, List<String> errorList) {
		...
		// !!!! but xerces doesn't support DOM level 2. When JDK goes to 1.4,
		// replace
		...
		if (item.getAttributeNode("value") != null) {
			...
			if (errorList != null && old_type != _type && old_type != TYPE_UNSPECIFIED)
				errorList.add("Type mismatch for key " + _key + ".  Specified type is " + _typeStrings[old_type]
						+ " but value is " + _typeStrings[_type]);
			if (errorList != null && nel != 0)
				errorList.add("Value attribute specified for key " + _key
						+ " but subitems are present.  Subitems ignored");
			...
			if (nel == 0) { // no subitems either!
				...
				int old_type = _type;
				_value = new ArrayList<String>(nel);
				for (int i = 0; i < nel; i++) {
					...
					if (!(subitem instanceof Element)) {
						...
						// !!!! THIS IS WHAT IT SHOULD BE:
						// !!!! if (errorList != null &&
						// !si.hasAttribute("value"))
						// !!!! but xerces doesn't support DOM level 2. When JDK
						// goes to 1.4, replace
						// !!!! the following with the above.
					...
					if (errorList != null && old_type != _type && old_type != TYPE_UNSPECIFIED)
						errorList.add("Type mismatch for key " + _key + ".  Specified type is "
								+ _typeStrings[old_type] + " but value is " + _typeStrings[_type]);
	...
	/***********************************************************************
	 * Returns a String representation of the given integer, suitable for a
	 * VICAR label.
	 */
	...
	/***********************************************************************
	 * Returns a String representation of the given float, suitable for a VICAR
	 * label.
	 */
	...
	/***********************************************************************
	 * Returns a String representation of the given double, suitable for a VICAR
	 * label.
	 */
	...
	/***********************************************************************
	 * Returns a String representation of the given string, suitable for a VICAR
	 * label. Quotes are provided around strings, and Internal single quotes are
	 * doubled. Note that strings are <em>not</em> stored this way inside the
	 * VicarLabelItem object. This should only be called when the string is
	 * being formatted for output to a label. This is a slight design
	 * inconsistency, since everything else is stored in ready-to-go string
	 * format.
	 */
	...
	/***********************************************************************
	 * Parses the given String according to the VICAR label rules, and returns a
	 * float. If the string is not a valid float, 0.0 is quietly returned.
	 */
	...
	/***********************************************************************
	 * Parses the given String according to the VICAR label rules, and returns a
	 * double. If the string is not a valid double, 0.0 is quietly returned.
	 */
	 ...
	 * Updates <code>_type</code> appropriately. <code>_type</code> will only be
	 * "promoted", e.g. unspecified->int->real->string, never the reverse...
	 * presumably other elements in the value already constrain it.
	 * <p>
	 * If the type is already specified, this routine ensures that the string is
	 * in fact valid for that type. If not, the type is quietly demoted to the
	 * appropriate less-restrictive type.
	 * <p>
	 * Integer characters are <code>+-0123456789</code> Float characters are
	 * <code>+-012345678.eEdD</code> Double is never guessed by this routine.
	 */
	 ...
	 * there. After all, the user might have just replaced the only value that
	 * was invalid for TYPE_INTEGER, for example.
	 */
	...
	/***********************************************************************
	 * Creates a deep copy of the object. Since _value is a vector of String, we
	 * don't need to copy the strings themselves, just the vector.
	 */
	...
	public Object clone() {
		...
		item._value = (ArrayList<String>) ((ArrayList<String>) (item._value)).clone();
