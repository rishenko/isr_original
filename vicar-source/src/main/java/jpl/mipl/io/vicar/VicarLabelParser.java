...
public class VicarLabelParser {
	...
	public VicarLabelItem getNextLabel() throws IOException, VicarLabelSyntaxException {
		...
		if (ttype != StreamTokenizer.TT_WORD && _tokenizer.sval == null) {
			return new VicarLabelItem("", "", ttype == '\'');
		}
		...
		ttype = _tokenizer.nextToken();
		if (ttype != '=')
			throw new VicarLabelSyntaxException("'=' expected after label keyword '" + keyword + "'");

		// Get the value.  Can be either a string, a word token, or a (.

		ttype = _tokenizer.nextToken();
		if (ttype != StreamTokenizer.TT_WORD && ttype != '\'' && ttype != '(')
			throw new VicarLabelSyntaxException("Invalid label value for keyword '" + keyword + "'");
		...
		while (ttype != ')') {
			...
			ttype = _tokenizer.nextToken();
			if (ttype != StreamTokenizer.TT_WORD && ttype != '\'')
				throw new VicarLabelSyntaxException("Invalid label value for multivalued keyword '" + keyword + "'");
			...
			ttype = _tokenizer.nextToken();
			if (ttype != ',' && ttype != ')')
				throw new VicarLabelSyntaxException("Missing comma or end paren for multivalued keyword '" + keyword
						+ "'");
