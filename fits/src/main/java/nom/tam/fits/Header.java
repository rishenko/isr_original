 ...
 * 
 * As of version 1.1 this class supports the long keyword convention which
 * allows long string keyword values to be split among multiple keywords
 * 
 * <pre>
 *    KEY        = 'ABC&'   /A comment
 ...
 * careful not to interpose new header cards within a long value sequence.
 * 
 * When writing long strings, the comment is included in the last card. If a
 * user is writing long strings, a the keyword LONGSTRN = 'OGIP 1.0' should be
 * added to the FITS header, but this is not done automatically for the user.
 */
public class Header implements FitsElement {
	 ...
	 * hash removes duplicate keys the internal size may be smaller.
	 * 
	...
	public static Header readHeader(ArrayDataInput dis) throws TruncatedFileException, IOException {
		...
		try {
			logger.trace("about to read header");
			myHeader.read(dis);
			logger.trace("finished reading header");
	...
	public void read(ArrayDataInput dis) throws TruncatedFileException, IOException {
		...
		if (dis instanceof RandomAccess) {
			logger.trace("finding offset");
			fileOffset = FitsUtil.findOffset(dis);
			logger.trace("finished finding offset");
		} else {
			fileOffset = -1;
		}
		...
		try {
			while (true) {
				logger.trace("reading header card");
				int len;

				int need = 80;

				try {
					logger.trace("read loop for input stream " + dis.getClass());
					while (need > 0) {
						logger.trace("before read");
						len = dis.read(buffer, 80 - need, need);
						logger.trace("after read");
						count += 1;
						if (len == 0) {
					...
					logger.trace("finished read loop for input stream");
				} catch (EOFException e) {
				...
				logger.trace("finished reading header card");
			}
