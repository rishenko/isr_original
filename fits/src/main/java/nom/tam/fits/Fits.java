...
public class Fits {
	...
	Logger logger = LoggerFactory.getLogger(Fits.class);
	 ...
	 * 
	 * The string is assumed to be a URL if it begins one of the protocol
	 * strings. If the string ends in .gz it is assumed that the data is in a
	...
	/**
	 * Read the first header found in the FITS file
	 * 
	 * @return
	 * @throws FitsException
	 * @throws IOException
	 */
	public Header readFirstImageHeader() throws FitsException, IOException {
		...
		if (ImageHDU.isHeader(hdr) && (hdr.getLongValue("NAXIS") != 0L || hdr.getLongValue("NAXIS1") != 0L)) {
			logger.debug("found image hdr");
			return hdr;
		}
		...
		logger.debug("no image hdr found, moving to the next hdr");
		return readFirstImageHeader();
	}
	...
	public BasicHDU readHDU() throws FitsException, IOException {
		...
		Data datum = hdr.makeData();
		try {
			logger.debug("reading datum");
			datum.read(dataStr);
			logger.debug("finished reading datum");
		} catch (PaddingException e) {
			e.updateHeader(hdr);
			throw e;
		}
	 ...
	 * @since 2005-10-05
	 */
	public static void setChecksum(BasicHDU hdu) throws nom.tam.fits.HeaderCardException, nom.tam.fits.FitsException,
			java.io.IOException {
		/*
