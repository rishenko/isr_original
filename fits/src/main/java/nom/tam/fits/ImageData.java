...
public class ImageData extends Data {
	...
	protected ArrayDesc parseHeader(Header h) throws FitsException {
		...
		logger.trace("finished parsing header");
	...
	public void read(ArrayDataInput i) throws FitsException {
		...
		if (i instanceof RandomAccess) {
			...
			logger.debug("creating data array");
			dataArray = ArrayFuncs.newInstance(dataDescription.type, dataDescription.dims);
			logger.debug("finished creating data array");
			try {
				logger.debug("about to read data array");
				i.readLArray(dataArray);
				logger.debug("finished reading data array");
			} catch (IOException e) {
		...
		try {
			...
			logger.error("eof received while trying to skip {} bytes for image padding", pad);
