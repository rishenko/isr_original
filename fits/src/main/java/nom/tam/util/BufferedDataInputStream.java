 ...
 * 
 * Version 1.1: October 12, 2000: Fixed handling of EOF to return partially read
 * arrays when EOF is detected. Version 1.2: July 20, 2009: Added handling of
 * very large Object arrays. Additional work is required to handle very large
 * arrays generally.
 */
public class BufferedDataInputStream extends BufferedInputStream implements ArrayDataInput {

	Logger logger = LoggerFactory.getLogger(BufferedDataInputStream.class);
	...
	public int read(byte[] obuf, int offset, int len) throws IOException {
		...
		while (len > 0) {
			...
			logger.trace("before read in stream");
			int xlen = super.read(obuf, offset, len);
			logger.trace("just after super.read");
			if (xlen <= 0) {
				if (total == 0) {
			...
			logger.trace("after read in stream");
		}
		return total;
	...
	public int read(int[] i, int start, int len) throws IOException {
		...
		try {
			for (; ii < start + len; ii += 1) {
				...
				i[ii] = buf[pos] << 24 | (buf[pos + 1] & 0xFF) << 16 | (buf[pos + 2] & 0xFF) << 8
						| (buf[pos + 3] & 0xFF);
				pos += 4;
			}
		} catch (EOFException e) {
	...
	public int read(long[] l, int start, int len) throws IOException {
		...
		try {
			for (; i < start + len; i += 1) {
				...
				int i1 = buf[pos] << 24 | (buf[pos + 1] & 0xFF) << 16 | (buf[pos + 2] & 0xFF) << 8
						| (buf[pos + 3] & 0xFF);
				int i2 = buf[pos + 4] << 24 | (buf[pos + 5] & 0xFF) << 16 | (buf[pos + 6] & 0xFF) << 8
						| (buf[pos + 7] & 0xFF);
	...
	public int read(float[] f, int start, int len) throws IOException {
		...
		try {
			for (; i < start + len; i += 1) {
				...
				int t = buf[pos] << 24 | (buf[pos + 1] & 0xFF) << 16 | (buf[pos + 2] & 0xFF) << 8
						| (buf[pos + 3] & 0xFF);
