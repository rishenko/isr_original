...
public class BufferedFile implements ArrayDataInput, ArrayDataOutput, RandomAccess {
	 ...
	 * 
	 * 
	 * /** Read an entire byte array. Note BufferedFile will return a partially
	 * filled array only at an end-of-file.
	 * 
	 * @param buf
	...
	private long convertToLong() throws IOException {
		...
		int i2 = buffer[x + 4] << 24 | (buffer[x + 5] & 0xFF) << 16 | (buffer[x + 6] & 0xFF) << 8
				| (buffer[x + 7] & 0xFF);
		bufferOffset += 8;
	...
	protected long primitiveArrayRecurse(Object o) throws IOException {
		System.out.println("enter primitive array recurse");
		if (o == null) {
		...
		if (className.charAt(1) == '[') {
			for (int i = 0; i < ((Object[]) o).length; i += 1) {
				System.out.println("recursively entering primitive array " + i);
			...
			case 'S':
				...
				System.out.println("read count " + primitiveArrayCount);
				break;
			case 'I':
	 ...
	 * writeBytes).
	 * 
	 * If the array is an array of objects, then write will be called for each
